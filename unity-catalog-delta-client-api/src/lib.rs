...
//! models for the connector-driven endpoints (`load_table`,
//! `staging-tables`, `tables`, credentials, `/config`). Concrete HTTP
//! implementations live in `unity-catalog-delta-rest-client`.
//!
//! # Trait scope is narrow by design
//!
//! Only `update_table` (the commit RPC) is behind a trait. Read, CREATE, and
...
pub use models::{
    ...
    CreateTableRequest, CreateTableResponse, LoadTableResponse, Protocol, Requirement,
    TableMetadata, Update, UpdateTableRequest, UpdateTableResponse,
};
