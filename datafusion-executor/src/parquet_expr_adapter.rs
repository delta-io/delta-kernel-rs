...
impl PhysicalExprAdapterFactory for KernelParquetExprAdapterFactory {
    fn create(
        ...
        // The default adapter must see the aligned schema or it rejects legacy fields before our
        // value-aware expression can validate them.
...
// Runs DataFusion's default rewrite, then inserts `ReconcileKernelNullabilityExpr` around columns
// that need nested nullability repair. For example, `cast(column)` becomes
// `cast(reconcile_kernel_nullability(column))`, so reconciliation executes before the cast.
...
///
/// Top-level fields remain physical so the default adapter performs the final conversion to the
/// requested Kernel fields. Nested fields are checked when their enclosing arrays are rebuilt
...
fn align_nested_nullability(logical_schema: &Schema, physical_schema: &Schema) -> Schema {
    ...
    for physical_field in physical_schema.fields() {
        let Some((_, logical_field)) = logical_schema.fields().find(physical_field.name()) else {
            aligned_fields.push(Arc::clone(physical_field));
            continue;
        };
        let data_type =
            align_nullable_data_types(logical_field.data_type(), physical_field.data_type());
...
fn align_nullable_data_types(logical_type: &DataType, physical_type: &DataType) -> DataType {
    match (logical_type, physical_type) {
        (DataType::Struct(logical_fields), DataType::Struct(physical_fields)) => {
            ...
            for physical_field in physical_fields {
                ...
                let data_type = align_nullable_data_types(
                    logical_field.data_type(),
                    physical_field.data_type(),
                );
                let field = align_nullable_field(logical_field, physical_field, data_type);
                aligned_fields.push(field);
            }
            DataType::Struct(aligned_fields.into())
        }
        (DataType::List(logical_field), DataType::List(physical_field)) => {
            let data_type =
                align_nullable_data_types(logical_field.data_type(), physical_field.data_type());
            let field = align_nullable_field(logical_field, physical_field, data_type);
            DataType::List(field)
        }
        (DataType::Map(logical_field, _), DataType::Map(physical_field, physical_keys_sorted)) => {
            let data_type =
                align_nullable_data_types(logical_field.data_type(), physical_field.data_type());
            let field = align_nullable_field(logical_field, physical_field, data_type);
...
fn align_nullable_field(
    logical_field: &FieldRef,
    physical_field: &FieldRef,
    data_type: DataType,
) -> FieldRef {
    let is_nullable = physical_field.is_nullable() && logical_field.is_nullable();
...
// DataFusion calls this implementation to evaluate the inserted reconciliation node against each
...
mod tests {
    ...
    fn reconciling_before_default_rewrite_places_cast_inside_reconciliation() {
        let (logical_schema, physical_schema, batch) = schema_evolution_case();
        let aligned_schema = Arc::new(align_nested_nullability(&logical_schema, &physical_schema));
        ...
        let adapted =
            DefaultPhysicalExprAdapter::new(logical_schema, aligned_schema).rewrite(reconciled);
    ...
    fn default_rewrite_preserves_type_casts_and_fills_missing_fields() {
        ...
        assert_eq!(array.data_type(), logical_schema.field(0).data_type());
    ...
    fn schema_evolution_case() -> (SchemaRef, SchemaRef, RecordBatch) {
        ...
        let logical_schema = Arc::new(Schema::new(vec![Field::new_struct(
            "action",
            logical_fields,
            true,
        )]));
        ...
        (logical_schema, physical_schema, batch)
    }
