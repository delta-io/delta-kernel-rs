...
> it, and it is built/tested on its own (from its directory, or with `--manifest-path`).

```bash
# Build
cargo build --workspace --all-features

# Build & test the datafusion_executor crate (separate workspace, see note above)
cargo build --manifest-path datafusion-executor/Cargo.toml
cargo test --manifest-path datafusion-executor/Cargo.toml
