...
`UCCommitter` is generic over `UCCommitsClient`, so different transport implementations
can be injected -- for example, the default REST client, a C++ FFI client, or a gRPC client.

### `UCCommitsClient` (trait)

Defines the interface for UC commit operations:

- `get_commits` -- fetch committed versions for a table (used during snapshot loading)
- `commit` -- register a new staged commit with UC
...
from UC via a `UCCommitsClient`, translates them into kernel `LogPath`s, and delegates to
