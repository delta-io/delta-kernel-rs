    ...
    CommitRequest request)
{
    ...
    if (request.commit_info.tag == SomeCommit) {
        ...
        // The UC API commit request carries no table URI, so fall back to the table root the
        // committer was created for (stored in the context).
        const char* staging_root = (request.table_uri.len > 0) ? table_uri
