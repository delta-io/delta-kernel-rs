...
uintptr_t engine_to_kernel_expression_item(
    KernelExpressionVisitorState* state,
    ExpressionItem item);

uintptr_t engine_to_kernel_literal(
    KernelExpressionVisitorState* state,
    struct Literal* lit) {
  switch (lit->type) {
    case Integer:
    ...
    default:
      fprintf(stderr,
          "Error: Unknown literal type in engine_to_kernel_literal\n");
      assert(0 && "Unknown literal type in engine_to_kernel_literal");
      abort(); // Explicitly abort even if assertions are disabled
  }
}

uintptr_t engine_to_kernel_binop(
    KernelExpressionVisitorState* state,
    struct BinOp* binop) {
  assert(binop->exprs.len == 2);
  uintptr_t left = engine_to_kernel_expression_item(
      state, binop->exprs.list[0]);
  uintptr_t right = engine_to_kernel_expression_item(
  ...
  switch (binop->op) {
    ...
    default:
      fprintf(stderr, "Error: Unknown binary op in engine_to_kernel_binop\n");
      assert(0 && "Unknown binary op in engine_to_kernel_binop");
      abort(); // Explicitly abort even if assertions are disabled
  }
}
...
const void* engine_to_kernel_next_fn(void* data) {
  ...
  uintptr_t result = engine_to_kernel_expression_item(iter_state->state, item);
  // Return the result as a pointer (cast uintptr_t to void*)
  return (const void*)result;
}

uintptr_t engine_to_kernel_variadic(
    KernelExpressionVisitorState* state,
  ...
  EngineIterator iterator = {
    .data = &iter_state,
    .get_next = engine_to_kernel_next_fn
  };
  ...
  switch (variadic->op) {
    ...
    default:
      fprintf(stderr,
          "Error: Unknown variadic op in engine_to_kernel_variadic\n");
      assert(0 && "Unknown variadic op in engine_to_kernel_variadic");
      abort(); // Explicitly abort even if assertions are disabled
  }
}

uintptr_t engine_to_kernel_unary(
    KernelExpressionVisitorState* state,
    struct Unary* unary) {
  assert(unary->sub_expr.len == 1);
  uintptr_t inner = engine_to_kernel_expression_item(
  ...
  switch (unary->type) {
    ...
    default:
      fprintf(stderr, "Error: Unknown unary op in engine_to_kernel_unary\n");
      assert(0 && "Unknown unary op in engine_to_kernel_unary");
      abort(); // Explicitly abort even if assertions are disabled
  }
}
...
uintptr_t engine_to_kernel_expression_item(
  ...
  switch (item.type) {
    case Literal:
      return engine_to_kernel_literal(state, (struct Literal*)item.ref);
    case BinOp:
      return engine_to_kernel_binop(state, (struct BinOp*)item.ref);
    case Variadic:
      return engine_to_kernel_variadic(state, (struct Variadic*)item.ref);
    case Unary:
      return engine_to_kernel_unary(state, (struct Unary*)item.ref);
    case Column: {
    ...
    default:
      fprintf(stderr,
          ...
          "engine_to_kernel_expression_item\n");
      assert(0 &&
          "Unknown expression type in engine_to_kernel_expression_item");
      abort(); // Explicitly abort even if assertions are disabled
  }
}
...
static inline uintptr_t expression_item_list_visitor(
  ...
  return engine_to_kernel_expression_item(state, expr_list->list[0]);
}
 ...
 */
SharedExpression* engine_to_kernel_expression(
  ...
  if (result.tag == OkHandleSharedExpression) {
    ...
    fprintf(stderr,
        "Error: Failed to convert engine expression to "
        "kernel expression\n");
    assert(0 &&
        "Failed to convert engine expression to kernel expression");
    abort(); // Explicitly abort even if assertions are disabled
  }
}
...
static inline uintptr_t predicate_item_list_visitor(
  ...
  return engine_to_kernel_expression_item(state, pred_list->list[0]);
}
 ...
 * hiding KernelExpressionVisitorState management from the caller.
 */
SharedPredicate* engine_to_kernel_predicate(
  ...
  if (result.tag == OkHandleSharedPredicate) {
    ...
    fprintf(stderr,
        "Error: Failed to convert engine predicate to "
        "kernel predicate\n");
    assert(0 &&
        "Failed to convert engine predicate to kernel predicate");
    abort(); // Explicitly abort even if assertions are disabled
  }
}
