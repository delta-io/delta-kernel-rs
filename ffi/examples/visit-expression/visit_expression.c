...
#include "reconstruct_kernel_expression.h"
#include <stdbool.h>
...
bool run_test_case(const TestCase* test) {
  ...
  if (test->validate_roundtrip) {
    SharedExpression* expr_rekernel = 
        reconstruct_kernel_expression(expr_list);
  ...
  if (test->validate_roundtrip) {
    SharedPredicate* pred_rekernel = 
        reconstruct_kernel_predicate(pred_list);
  ...
  TestCase test_cases[] = {
    {
      ...
      .validate_roundtrip = false,   // FIXME: This is not supported yet
      .description = 
