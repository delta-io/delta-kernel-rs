...
/// Creates a new expression evaluator as provided by the passed engines `ExpressionHandler`.
///
/// # Safety
...
fn new_expression_evaluator_impl(
    ...
    let engine = extern_engine.engine();
    let evaluator = engine.expression_handler().new_expression_evaluator(
        input_schema,
        expression.clone(),
        output_type,
    );
