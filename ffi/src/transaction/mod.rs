...
use delta_kernel::transaction::{
    CommitActions, CommitResult, CommittedTransaction, CreateTable, PreparedCommit, Transaction,
    TransactionBuilder, TransactionOptions,
};
...
impl FfiTransaction {
    ...
    pub(super) fn transaction(&mut self, engine: &dyn Engine) -> DeltaResult<&Transaction> {
        if self.transaction.is_none() {
            ...
            let builder = builder.with_options(std::mem::take(&mut self.options));
            self.transaction = Some(builder.build(engine)?);
        }
...
/// [`commit_result_into_committed`] and recover the exact prepared commit from conflicted or
/// retryable results with [`commit_result_into_prepared_commit`].
#[handle_descriptor(target=FfiCommitResult, mutable=true, sized=true)]
pub struct ExclusiveCommitResult;

/// A handle retaining an immutable transaction and its exact commit actions.
///
/// Retry with [`prepared_commit_retry`] or release with [`free_prepared_commit`]. A prepared commit
/// recovered from a conflict requires rebasing before the same commit version can succeed; no FFI
/// rebase operation is currently available.
...
fn into_ffi_commit_result<S>(
    ...
    match result? {
        ...
        CommitResult::ConflictedTransaction(conflicted) => {
            ...
            let prepared_commit = wrap_prepared(conflicted.into_prepared_commit());
...
/// # Safety
///
/// Caller is responsible for passing valid handles. CONSUMES the result handle.
#[no_mangle]
...
/// Consume a conflicted or retryable result and return its exact prepared commit.
///
/// # Errors
///
/// Returns an error if `result` is committed.
///
/// # Safety
///
/// Caller is responsible for passing valid handles. CONSUMES the result handle.
...
///
/// Returns a new commit-result handle. A prepared commit recovered from a conflict will conflict
/// again until it is rebased; no FFI rebase operation is currently available.
///
/// # Safety
///
/// Caller is responsible for passing valid handles. CONSUMES the prepared-commit handle.
#[no_mangle]
...
/// Set whether files supplied to a create-table commit represent a logical data change.
///
/// This consumes the builder handle and returns a new one. The caller MUST replace their handle
...
/// # Safety
///
/// Caller is responsible for passing a valid builder handle. CONSUMES the builder handle.
#[no_mangle]
pub unsafe extern "C" fn create_table_builder_with_data_change(
    builder: Handle<ExclusiveCreateTableBuilder>,
    data_change: bool,
...
mod tests {
    ...
    #[tokio::test]
    async fn conflicted_commit_result_retains_prepared_commit(
    ) -> Result<(), Box<dyn std::error::Error>> {
        let schema = schema_ref! { nullable "number": INTEGER };
        ...
        let prepared = ok_or_panic(unsafe {
            commit_result_into_prepared_commit(conflicted, engine.shallow_copy())
        });

        let retried =
            ok_or_panic(unsafe { prepared_commit_retry(prepared, engine.shallow_copy()) });
        assert_eq!(
            unsafe { commit_result_kind(&retried) },
            KernelCommitResultKind::Conflicted
        );
        let prepared = ok_or_panic(unsafe {
            commit_result_into_prepared_commit(retried, engine.shallow_copy())
        });
