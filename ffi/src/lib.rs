...
pub unsafe extern "C" fn snapshot(
    ...
    // Only parse URL if we don't have an old snapshot
    let url = if old_snapshot_handle.is_none() {
        match path {
            OptionalValue::Some(p) => Some(unsafe { unwrap_and_parse_path_as_url(p) }),
            OptionalValue::None => {
