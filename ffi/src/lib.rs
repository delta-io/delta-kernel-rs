...
fn get_default_engine_impl(
    ...
    let engine: Arc<dyn Engine> = if let Some(config) = executor_config {
        ...
        Arc::new(DefaultEngineBuilder::new(prefixed).build())
    };
...
mod tests {
    ...
    > {
        ...
        let engine = engine_to_handle(
            Arc::new(
                DefaultEngineBuilder::new(PrefixedStore::new(storage.clone(), Path::from("")))
                    .build(),
            ),
            allocate_err,
        );
    ...
    async fn test_snapshot_checkpoint() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let engine = DefaultEngineBuilder::new(PrefixedStore::new(storage.clone(), Path::from("")))
    ...
    async fn test_builder_from_table_path_builds_latest_version(
        ...
        let engine = engine_to_handle(
            Arc::new(
                DefaultEngineBuilder::new(PrefixedStore::new(storage, Path::from(""))).build(),
            ),
            allocate_err,
        );
