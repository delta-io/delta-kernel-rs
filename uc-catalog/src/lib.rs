...
use delta_kernel::{Engine, FileMeta, LogPath, Snapshot, Version};
...
impl UCCatalog {
    ...
    pub(crate) async fn load_snapshot_inner(
        ...
        // TODO: do we need to sort? paginate?
        let commits = self.client.get_commits(req).await?;
        println!("[GET COMMITS] get commits response: {:?}\n", commits);
        ...
        let parse_commit = |commit: Commit| -> Result<LogPath, Box<dyn std::error::Error>> {
            info!("[PARSE COMMIT] parsing commit: {:?}", commit);
            // FIXME
            let location = table_uri.clone() + "/_delta_log/_staged_commits/" + &commit.file_name;
            let file_meta = FileMeta {
                location: Url::parse(&location)?,
                last_modified: commit.file_modification_timestamp,
                size: commit.file_size.try_into()?,
            };
            LogPath::try_new(file_meta).map_err(|e| e.into())
        };
