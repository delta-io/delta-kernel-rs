...
mid-read. Whenever cancellation is observed, the read surfaces `EngineError::cancelled()` as a
terminal error — never a short or empty result that could be mistaken for a complete one.
