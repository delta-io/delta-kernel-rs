...
as a `PreparedCommit` and attempts the commit. Retryable results retain that executable prepared
commit. Conflicted results retain the stale transaction and actions for inspection and action
recovery, but require rebuilding the transaction against a fresh snapshot. Neither result retains
the writer contexts.
...
logical independence from existing data. Kernel does not automatically rebase concurrent blind
appends, so a commit-version race can still return `ConflictedTransaction`. The connector must
