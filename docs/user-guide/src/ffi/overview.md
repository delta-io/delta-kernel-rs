...
commit_result_kind()
        |
        +-- committed -> commit_result_into_committed()
        +-- retryable -> commit_result_into_prepared_commit()
        +-- conflicted -> rebuild from a fresh snapshot
```
...
| `commit_result_into_prepared_commit` | Consume a retryable result and recover its exact prepared commit |
