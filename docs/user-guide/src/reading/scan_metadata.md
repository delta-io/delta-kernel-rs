...
row transforms used by `Scan::execute`. Checkpoint footer pruning continues to use the checkpoint's
native parsed partition values. Incremental scans expose the raw partition-value map. For
daylight-saving transitions, ambiguous local times use the earlier instant, and nonexistent local
times use the offset from before the transition.
