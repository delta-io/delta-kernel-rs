...
pub fn into_struct_data_derive(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    // Fields marked `#[allow_null_container_values]` get a value-nullable map type from `ToSchema`,
    // so the value must be made value-nullable too, otherwise the schema and value types disagree.
