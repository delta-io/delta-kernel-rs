...
`Snapshot` -> `ExistingTableTransactionBuilder` -> `Transaction` -> (`WriteState` ->
...
**Data-write steps:**
1. Configure an `ExistingTableTransactionBuilder` from a snapshot and build it with a `Committer`
