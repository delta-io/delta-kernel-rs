...
`UCClient` and `UCCommitsRestClient` (in the `delta-kernel-unity-catalog` crate).

### `http`

HTTP utilities: client construction (`build_http_client`), retry logic (`execute_with_retry`),
and response handling (`handle_response`). These are re-used by the commits REST client in
