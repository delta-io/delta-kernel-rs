...
/// The schema-evolution ALTER under test.
enum SchemaAlter {
    AddColumn,
    SetNullable,
}

/// Schema evolution (`add_column` / `set_nullable`) through the committer forwards a whole-schema
/// `SetColumns` update, and the evolved schema is visible after reloading from the catalog.
#[rstest]
#[case::add_column(SchemaAlter::AddColumn)]
#[case::set_nullable(SchemaAlter::SetNullable)]
#[tokio::test(flavor = "multi_thread")]
async fn test_alter_schema_evolution_forwards_set_columns(
    #[case] op: SchemaAlter,
) -> Result<(), TestError> {
    ...
    // `id` is not-null so `set_nullable` has an observable effect.
    ...
    create_table(table_uri.as_str(), schema, "delta-kernel-uc-test")
        ...
        .build(&engine, Box::new(uc_committer(&client)))?
        .commit(&engine)?
        .unwrap_committed();
    ...
    let alter = snap_v0.alter_table();
    let alter = match op {
        SchemaAlter::AddColumn => alter.add_column(StructField::nullable("extra", DataType::STRING)),
        SchemaAlter::SetNullable => alter.set_nullable(column_name!("id")),
    };
    alter
        .build(&engine, Box::new(uc_committer(&client)))?
        .commit(&engine)?
    ...
    // The committer forwarded a whole-schema `SetColumns` carrying the evolved schema.
    ...
    let columns = last
        ...
        .iter()
        .find_map(|u| match u {
            Update::SetColumns { columns } => Some(columns),
            _ => None,
        })
        .ok_or("expected a SetColumns update")?;
    let fields = columns["fields"]
        .as_array()
        .ok_or("SetColumns.columns has no fields array")?;

    // The evolved schema is visible on reload (read from the staged commit's metaData).
    let reloaded = build_snapshot(&client, &engine, &table_uri)?;
    assert_eq!(reloaded.version(), 1);
    let schema = reloaded.schema();
    match op {
        SchemaAlter::AddColumn => {
            assert!(
                fields.iter().any(|f| f["name"] == "extra"),
                "SetColumns must carry the added column, got {fields:?}"
            );
            assert!(
                schema.field("extra").is_some(),
                "reloaded schema must contain the added column"
            );
        }
        SchemaAlter::SetNullable => {
            let id = fields
                .iter()
                .find(|f| f["name"] == "id")
                .ok_or("SetColumns missing the `id` field")?;
            assert_eq!(id["nullable"], serde_json::Value::Bool(true));
            assert!(
                schema.field("id").ok_or("reloaded schema missing `id`")?.is_nullable(),
                "reloaded `id` must be nullable"
            );
        }
    }
    Ok(())
