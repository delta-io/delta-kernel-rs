...
This crate also provides shared infrastructure (`config`, `error`, `http`) used by both
itself and the `unity-catalog-client` crate.
...
Shared configuration (workspace URL, auth token, timeout, retry settings) used by both
`UCCommitsRestClient` (in this crate) and `UCClient` (in the `unity-catalog-client` crate).

### `error`

Error types for UC API operations (`ApiError`, `AuthenticationFailed`, `TableNotFound`,
`MaxUnpublishedCommitsExceeded`, etc.) and a `Result` type alias.

### `http`

HTTP utilities: client construction (`build_http_client`), retry logic (`execute_with_retry`),
and response handling (`handle_response`).
