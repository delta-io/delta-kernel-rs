...
2. Add col and lit expression builders ([#2684])
3. Added get_earliest_delta_file function ([#2582])
4. Introduce FFI types for plan execution ([#2680])
5. Add MeteredJsonHandler and MeteredParquetHandler ([#2694])
6. Use incremental CRC replay in Snapshot construction ([#2678])
7. Add FFI-backed PlanExecutor impl ([#2685])
8. Support incremental CRC reconstruction on snapshot `builder_from` API ([#2712])
9. Support local DAT corpus override for acceptance workloads ([#2705])
10. Add proto wire format for declarative-plans IR ([#2629])
11. Add a public history_manager::get_earliest_commit ([#2713])
12. Define and use a new projection struct patch ([#2730])

### 🐛 Bug Fixes

1. Specify selection-vector contracts at the engine boundary ([#2728])
2. Restore CRC-rooted P&M replay removed by #2678 ([#2741])
...
3. Update benchmark output format, update tables ([#2703])
4. Unify column mapping validations to `MakePhysical` ([#2599])
5. Eagerly materialize the physical no-partition-schema ([#2743])
...
3. Fixup title and body validator actions for merge/workflow ([#2697])
4. Fix dupliate CI runs ([#2704])
5. Various improvements ([#2708])
6. Bump codecov-action to v7.0.0 to fix coverage uploads ([#2711])
7. Fix flaky metrics deadlock test ([#2716])
8. Make delta_kernel_default_engine publishable ([#2561])
9. *(deps)* Bump rustls-webpki from 0.103.10 to 0.103.13 ([#2735])
10. Run body validator on synchronize and reopened ([#2748])

### Other

1. Update release
2. Release 0.25.0
3. Try to get the breaking-change work
4. Release 0.25.0
...
[#update release]: https://github.com/delta-io/delta-kernel-rs/pull/update release
[#release 0.25.0]: https://github.com/delta-io/delta-kernel-rs/pull/release 0.25.0
[#try to get the breaking-change work]: https://github.com/delta-io/delta-kernel-rs/pull/try to get the breaking-change work
[#release 0.25.0]: https://github.com/delta-io/delta-kernel-rs/pull/release 0.25.0
