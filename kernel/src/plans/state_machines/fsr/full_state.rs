...
mod tests {
    ...
    #[test]
    fn scan_data_projection_maps_partition_columns_from_parsed_values() {
        let logical = Arc::new(StructType::new_unchecked([
            StructField::nullable("part", DataType::STRING),
            StructField::not_null("value", DataType::LONG),
        ]));
        let partition_columns = HashSet::from([String::from("part")]);
        let projection = scan_data_projection(
            &logical,
            &logical,
            &partition_columns,
        ...
        assert!(matches!(
            projection[0].as_ref(),
            Expression::Column(c)
                if c == &ColumnName::new([FILE_CONSTANT_VALUES_NAME, "partitionValues_parsed", "part"])
        ));
        assert!(matches!(
            projection[1].as_ref(),
            Expression::Column(c) if c == &ColumnName::new(["value"])
    ...
    #[test]
    fn scan_data_projection_uses_physical_partition_key_for_column_mapping() {
        let logical = Arc::new(StructType::new_unchecked([
            StructField::nullable("part", DataType::STRING).with_metadata([(
                ColumnMetadataKey::ColumnMappingPhysicalName.as_ref(),
                MetadataValue::String("col-part-physical".to_string()),
            )]),
            StructField::not_null("value", DataType::LONG),
        ]));
        let partition_columns = HashSet::from([String::from("part")]);
        let projection = scan_data_projection(
            ...
            &logical,
            &partition_columns,
            crate::table_features::ColumnMappingMode::Name,
        );
        assert!(matches!(
            projection[0].as_ref(),
            Expression::Column(c)
    ...
    fn plan_sink_kind(plan: &Plan) -> PlanTerminalKind {
        match &plan.sink.sink_type {
            ...
            SinkType::Results(_) => PlanTerminalKind::Results,
