...
impl ScanBuilder {
    ...
    ///
    /// **Note: This method is mutually exclusive with [`with_stats_columns`].** Calling [`build`]
    /// with both set will return an error.
    ///
    ...
    ///
    /// **Note: This method is mutually exclusive with [`with_predicate`].** Use one or the other:
    /// - Use `with_predicate` when you want the kernel to perform data skipping automatically
    /// - Use `with_stats_columns` when you want to receive parsed stats and perform your own
    ///   data skipping logic
    ///
    /// Calling [`build`] with both `with_predicate` and `with_stats_columns` set will return
    /// an error.
    ///
