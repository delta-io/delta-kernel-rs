...
/// Builds a transform expression from a transform spec and field values.
/// This is a shared utility used by both regular scans and CDF scans.
pub(crate) fn build_transform_expr(
...
mod tests {
    ...
    mod transform_utils_tests {
        ...
        fn test_transform_utilities() {
            ...
            let result = parse_partition_values_to_expressions(&schema, &transform_spec, &partition_values);
            assert!(result.is_ok());
            ...
            let result = parse_partition_values_to_expressions(&schema, &transform_spec, &null_partition_values);
            assert!(result.is_ok());
            let values = result.unwrap();
            assert_eq!(values.len(), 2);
