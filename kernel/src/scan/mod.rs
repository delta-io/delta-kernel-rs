...
/// stat expressions that already carry their `stats_parsed.*` / `partitionValues_parsed.*` root
/// (e.g. `stats_parsed.minValues.x > 100`) with `add`, yielding `add.stats_parsed.minValues.x >
/// 100`.
