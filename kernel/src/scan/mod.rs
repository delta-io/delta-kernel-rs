...
use crate::schema::{
    lazy_schema_ref, DataType, LogicalSchema, LogicalSchemaRef, MapType, SchemaRef, StructField,
...
impl ScanBuilder {
    ...
    pub fn build(self) -> DeltaResult<Scan> {
        ...
        let raw_logical_read_schema = self
            .logical_read_schema
        ...
        let logical_read_schema = LogicalSchema::new(raw_logical_read_schema, table_config);
        // Predicates may reference columns outside the projection, so resolve them against the
        // full table schema.
        let table_schema = LogicalSchema::new(table_schema, table_config);
    ...
    pub fn execute(
        ...
        let result = scan_files_iter
            .map(move |scan_file| -> DeltaResult<_> {
                ...
                let logical_schema_inner = logical_schema.clone(); // Arc clone
