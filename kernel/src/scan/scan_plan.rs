...
impl Scan {
    ...
    pub(super) fn build_metadata_scan_plan(
        ...
        let prune = if self.skip_stats() {
            None
        } else {
            stats_skipping_predicate(state)
        };
    ...
    fn checkpoint_arm(&self, shape: &CheckpointShape) -> DeltaResult<PlanBuilder> {
        ...
        let native_partition_predicate = if self.skip_stats() {
            None
        } else {
            source_physical_partitions.and_then(|_| self.build_actions_partition_predicate())
        };
        actions
            // Preserve native checkpoint partition skipping. The reader timezone applies when
            // surviving rows are rebuilt from the raw map below, not to this early pruning stage.
            .try_fold_with(native_partition_predicate, |actions, predicate| {
