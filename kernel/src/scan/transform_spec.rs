...
mod tests {
    ...
    #[rstest::rstest]
    fn test_parse_partition_value_raw_empty_string_is_null(
        #[values(DataType::STRING, DataType::BINARY, DataType::INTEGER)] data_type: DataType,
    ) {
        let value = parse_partition_value_raw(Some(&String::new()), &data_type).unwrap();
        assert_eq!(value, Scalar::Null(data_type));
    }
