...
pub(crate) fn parse_partition_value_raw(
    ...
    match (raw, data_type.as_primitive_opt()) {
        (Some(value), Some(PrimitiveType::Timestamp)) if !value.is_empty() => {
            parse_partition_timestamp(value, &TimestampTimezone::default())
...
mod tests {
    ...
    #[test]
    fn test_parse_partition_value_raw_timestamp() {
        ...
        let numeric_offset = "2024-01-15T12:30:45+02:00".to_string();
        assert_eq!(
            parse_partition_value_raw(Some(&numeric_offset), &DataType::TIMESTAMP).unwrap(),
            Scalar::Timestamp(1_705_314_645_000_000)
        );

        let submicrosecond = "1970-01-01 00:00:00.1234567".to_string();
        assert_eq!(
            parse_partition_value_raw(Some(&submicrosecond), &DataType::TIMESTAMP).unwrap(),
            Scalar::Timestamp(123_456)
        );

        let invalid_timestamp = "not a timestamp".to_string();
        assert!(matches!(
            parse_partition_value_raw(Some(&invalid_timestamp), &DataType::TIMESTAMP),
        ...
        for (data_type, raw) in [
            ...
            assert!(matches!(
                parse_partition_value_raw(Some(&raw.to_string()), &data_type),
                Err(Error::ParseError(..))
            ));
        }
    }
