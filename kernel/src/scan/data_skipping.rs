...
impl DataSkippingFilter {
    ...
    fn build_unified_schema_and_expr(
        ...
        let unified_expr = match (has_stats, pv_schema, partition_expr) {
            (true, Some(_), Some(pv_expr)) => Arc::new(Expr::Struct(vec![stats_expr, pv_expr])),
            (false, Some(_), Some(pv_expr)) => Arc::new(Expr::Struct(vec![pv_expr])),
            _ => Arc::new(Expr::Struct(vec![stats_expr])),
        };
