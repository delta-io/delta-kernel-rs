...
pub(crate) use stats_schema::build_stats_schema;
...
impl DataSkippingFilter {
    ...
    pub(crate) fn apply(
        ...
        let final_stats = if !is_log_batch && self.select_stats_parsed_evaluator.is_some() {
            // Checkpoint batch with compatible stats_parsed - use it directly
            let stats_parsed_evaluator = self.select_stats_parsed_evaluator.as_ref().unwrap();
