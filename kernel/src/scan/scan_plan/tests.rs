...
#[case::predicate(
    StatsOptions::none(),
    Some(col!("age").gt(Expr::literal(0i64))),
)]
#[should_panic(expected = "checkpoint JSON stats must be retained for missing parsed columns")]
...
fn declarative_metadata_handles_absent_stats(
    ...
    let scan = snapshot
        .scan_builder()
        .with_stats(stats.clone())
        .with_predicate(Arc::new(col!("value").gt(Expr::literal(i32::MAX))))
        .build()?;
    ...
    for batch in &actual {
        ...
        if matches!(stats.struct_stats, StructStats::AllIndexed { .. }) {
            let parsed = batch
...
fn declarative_metadata_synthesizes_json_for_struct_only_checkpoints(
    ...
    let predicate = with_predicate.then(|| col!("clust_int").gt(Expr::literal(0i32)));
    assert_metadata_output_options(
        log_state,
        features,
        checkpoint_struct_stats(),
