...
impl ScanLogReplayProcessor {
    ...
    pub(crate) fn new_with_seen_files(
        ...
        // Build transform expressions and output schema with stats_parsed if available
        let stats_schema = state_info.stats_schema.clone();
        let output_schema = scan_row_schema_with_stats(stats_schema.clone());

        // Log transform: always uses ParseJson (log batches don't have stats_parsed)
        let log_transform_expr = get_add_transform_expr_with_stats(stats_schema.clone(), false);
        ...
        // Checkpoint transform: uses coalesce(stats_parsed, ParseJson) when checkpoint has compatible stats
        let checkpoint_transform_expr =
            get_add_transform_expr_with_stats(stats_schema, has_stats_parsed);
        ...
        // Create data skipping filter that reads stats_parsed from transformed batch
...
/// Build the add transform expression, optionally including stats_parsed.
///
/// When `stats_schema` is provided, the transform includes a `stats_parsed` column.
/// If `has_stats_parsed` is true (checkpoint has compatible pre-parsed stats), uses
/// `COALESCE(add.stats_parsed, ParseJson(add.stats))` to prefer existing stats.
/// Otherwise, just uses `ParseJson(add.stats)`.
fn get_add_transform_expr_with_stats(
    stats_schema: Option<SchemaRef>,
...
impl LogReplayProcessor for ScanLogReplayProcessor {
    ...
    fn process_actions_batch(&mut self, actions_batch: ActionsBatch) -> DeltaResult<Self::Output> {
        ...
        // Transform first to get stats_parsed in the batch
