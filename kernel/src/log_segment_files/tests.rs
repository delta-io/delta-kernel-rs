...
async fn list_commits_merges_log_tail(
    #[case] legacy_filesystem_commits: &[Version],
    #[case] staged: &[Version],
...
async fn test_list_ignoring_checkpoints_keeps_commits_and_crc() {
    ...
    let result = drive_storage(storage.as_ref(), None, async move |channel| {
        LogSegmentFiles::list_with_checkpoint_handling(
            &channel,
            &log_root,
            vec![],
            Some(0),
