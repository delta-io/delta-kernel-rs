...
pub enum LogHistoryError {
    ...
    /// The timestamp range contains no commits - the entire range falls between two adjacent versions.
    #[error(
    ...
    EmptyTimestampRange {
        ...
        end_timestamp: Timestamp,
        /// The version immediately before the timestamp range (the next version is `between_version + 1`).
