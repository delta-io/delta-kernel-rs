...
impl LogHistoryManager {
    ...
    ///   greatest time that this [`LogHistoryManager`] will be able to resolve timestamps for.
    /// - `limit`: Optional maximum number of versions to track in the history manager's state.
    ///   When specified, the earliest queryable version is `snapshot.version - limit`. This
    ///   parameter allows trading memory usage for historical reach.
    /// ```
    #[allow(unused)]
