...
mod test {
    ...
    fn test_catalog_managed_requires_in_commit_timestamp(
        ...
        let result = config.ensure_operation_supported(Operation::DataWrite(DataWriteOp::Append));
    ...
    ///
    /// Both features require `inCommitTimestamp` to be enabled, so the mock table enables it;
    /// otherwise the feature-requirement check would fail before the CDF cell is reached.
    #[rstest]
