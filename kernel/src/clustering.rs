...
/// 1. Columns are top-level (not nested paths)
/// 2. Columns exist in the schema
/// 3. Columns have data types eligible for statistics collection (required for clustering)
///
/// Note: The number of columns (1-4) should be validated before calling this function,
/// typically in `DataLayout::clustered()`.
///
/// # Errors
///
/// Returns an error if any column is nested, not found in the schema, or has an
