...
///
/// - **`DefaultEngine` consumers**: pass this to `DefaultEngine::write_parquet` (requires
///   `default-engine-base`), which handles everything (transform, write, partition metadata).
/// - **Arrow-based custom engines**: write parquet yourself, then call `build_add_file_metadata`
///   (requires `default-engine-base`) with the resulting `DataFileMetadata` and this
