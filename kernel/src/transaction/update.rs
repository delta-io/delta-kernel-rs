...
impl Transaction {
    ...
    /// When row tracking is enabled, selected rows must have non-null,
    /// non-negative `baseRowId` and `defaultRowCommitVersion` in `fileConstantValues`.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// When row tracking is enabled, matched rows must also have non-null,
    /// non-negative `baseRowId` and `defaultRowCommitVersion` in `fileConstantValues`.
    ///
