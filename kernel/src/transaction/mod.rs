...
pub(crate) static BASIC_ADD_FILE_SCHEMA: LazyLock<SchemaRef> = LazyLock::new(|| {
...
pub(crate) fn basic_add_file_schema() -> &'static SchemaRef {
    &BASIC_ADD_FILE_SCHEMA
}
...
pub(crate) static ADD_FILES_SCHEMA: LazyLock<SchemaRef> = LazyLock::new(|| {
    ...
    Arc::new(StructType::new(
        basic_add_file_schema().fields().cloned().chain([stats]),
    ))
});
...
// NOTE: The following two methods are a hotfix for the fact that we do not have a proper SchemaBuilder yet.
...
// NOTE: The following method is a hotfix for the fact that we do not have a proper SchemaBuilder yet.
...
impl Transaction {
    ...
    pub fn commit(self, engine: &dyn Engine) -> DeltaResult<CommitResult> {
            ...
            .is_row_tracking_suspended()
        {
            self.generate_adds(
                ...
                as_log_add_schema(with_stats_col(basic_add_file_schema())),
            )
        } else {
    ...
    fn generate_adds_with_row_tracking<'a>(
        ...
        // Read the current rowIdHighWaterMark from the snapshot's row tracking domain
        let row_id_high_water_mark = domain_metadata_configuration(
            self.read_snapshot.log_segment(),
            ROW_TRACKING_DOMAIN_NAME,
            engine,
        )?
        .map(|domain_metadata| serde_json::from_str::<RowTrackingDomainMetadata>(&domain_metadata))
        .transpose()?
        ...
        let domain_metadata = DomainMetadata::new(
            ...
            serde_json::to_string(&RowTrackingDomainMetadata {
                row_id_high_water_mark: Some(row_tracking_visitor.row_id_high_water_mark),
            })?,
            false,
        );
        ...
        let extended_add_files_metadata =
            self.add_files_metadata.iter().zip(base_row_id_batches).map(
                move |(add_files_batch, base_row_ids)| {
                    ...
                    add_files_batch.try_append_columns(
