...
impl<S> CommitActions<S> {
    ...
    pub fn is_empty(&self) -> bool {
        self.add_files_metadata.iter().all(|data| data.len() == 0)
            && self
                .remove_files_metadata
                .iter()
                .all(|data| !data.has_selected_rows())
            && self
                .dv_matched_files
                .iter()
                .all(|data| !data.has_selected_rows())
...
///
/// This is the unit retained by retryable and conflicted commit results. Retrying a prepared
/// commit therefore cannot accidentally restage, omit, or duplicate data files.
...
impl<S> Transaction<S> {
    ...
    pub fn prepare_commit(self, actions: CommitActions<S>) -> DeltaResult<PreparedCommit<S>> {
    ...
    /// - Ok(CommitResult) for either success or a recoverable error (includes the failed prepared
    ///   commit in case of a conflict or retryable failure)
    ...
    pub fn commit(
        ...
        self.prepare_commit(actions)?.commit(engine, committer)
    }
}
...
impl<S> PreparedCommit<S> {
    fn validate(&self) -> DeltaResult<()> {
        ...
        if !commit_actions.remove_files_metadata.is_empty() {
            transaction
                .effective_table_config
        ...
        if !commit_actions.add_files_metadata.is_empty() {
        ...
        if !transaction.is_create_table()
            && !commit_actions.add_files_metadata.is_empty()
            && (!commit_actions.remove_files_metadata.is_empty()
                || commit_actions.num_dv_updates > 0)
    ...
    ///
    /// Retryable and conflicted results retain this exact prepared commit.
    ///
...
impl<S> Transaction<S> {
    ...
    fn validate_committer(&self, committer: &dyn Committer) -> DeltaResult<()> {
        let commit_type =
    ...
    fn validate_blind_append_semantics(&self, actions: &CommitActions<S>) -> DeltaResult<()> {
        ...
        require!(
            !actions.add_files_metadata.is_empty(),
        ...
        require!(
            actions.remove_files_metadata.is_empty(),
    ...
    fn is_create_table(&self) -> bool {
        debug_assert!(
            self.operation.as_ref().map(Operation::as_str) != Some("CREATE TABLE")
...
impl<S> Transaction<S> {
    ...
    fn generate_adds<'a>(
        ...
        if add_files_metadata.is_empty() {
...
impl<S> ConflictedTransaction<S> {
    ...
    /// Return the exact prepared commit that encountered the conflict.
    pub fn prepared_commit(&self) -> &PreparedCommit<S> {
        &self.prepared_commit
    }

    /// Consume this result and recover the exact prepared commit for conflict handling.
...
mod tests {
    ...
    fn test_retry_preserves_exact_action_cardinality_without_duplication() -> DeltaResult<()> {
        ...
        assert!(!conflicted.prepared_commit().actions().is_empty());
        let prepared = conflicted.into_prepared_commit();
