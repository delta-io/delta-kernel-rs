...
pub(super) struct ExistingTransactionConfig {
    ...
    pub(super) effective_table_config: Option<TableConfiguration>,
    ...
    #[cfg(feature = "adaptive-metadata-in-dev")]
    pub(super) root_manifest_file: Option<FileMeta>,
}
...
impl Transaction {
    ...
    pub(super) fn try_new_existing_table(
        ...
        let should_emit_metadata = config.effective_table_config.is_some();
        let effective_table_config = config
            .effective_table_config
            .unwrap_or_else(|| read_snapshot.table_configuration().clone());
        #[cfg(feature = "adaptive-metadata-in-dev")]
        let root_manifest_file = config
            .root_manifest_file
            .map(|file| RootManifestFile::new(file, read_snapshot.clone()));
        ...
        Transaction::try_from_init(TransactionInit::<ExistingTable> {
            ...
            provided_row_tracking_high_water_mark: config.provided_row_tracking_high_water_mark,
...
impl<S> Transaction<S> {
    ...
    pub(super) fn generate_dv_update_actions<'a>(
        ...
        let remove_actions =
            self.generate_remove_actions(engine, attempt_timestamp, dv_matched_files.iter(), true)?;
