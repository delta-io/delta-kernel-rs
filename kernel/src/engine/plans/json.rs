...
use crate::{
    DeltaResultIterator, EngineData, EngineResult, FileDataReadResultIterator, FileMeta,
...
impl JsonHandler for PlanBasedJsonHandler {
    ...
    fn read_json_files(
        ...
        let query = PlanBuilder::scan_json(files.to_vec(), &[], physical_schema)
            ...
            .map_err(|error| super::adapter_error("JSON scan plan construction failed", error))?;
        let data = self
            .executor
