...
/// `timestamp_timezone` applies only to `TIMESTAMP` values without an explicit offset; it does not
/// affect `DATE` or `TIMESTAMP_NTZ`.
...
/// `timestamp_timezone` controls `TIMESTAMP` values without an explicit offset.
///
