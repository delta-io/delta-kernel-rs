...
// Footer min/max ignore the null partition sibling, so a non-matching value prunes the group.
#[test]
fn checkpoint_filter_partition_unguarded_prunes_group_with_null_sibling() {
    ...
    // Non-matching value outside the footer range -> prune, including the null sibling.
    assert!(!CheckpointRowGroupFilter::apply(
        row_group,
...
// === End-to-end row-group I/O tests ===

/// One checkpoint row's stats: a unique `id`, a data column `x` (min/max), and a partition value.
/// `None` writes a real parquet null with the same partition-leaf footer stats as a non-Add row.
struct RgSpec {
    id: i64,
    x_min: i64,
    x_max: i64,
...
/// forces the boundary). A single-row group models one Add; a multi-row group can model an Add
/// sharing a row group with a null partition leaf.
fn write_checkpoint(groups: &[&[RgSpec]]) -> tempfile::NamedTempFile {
    write_checkpoint_with_partition_values(groups, true)
}

fn write_checkpoint_with_partition_values(
    groups: &[&[RgSpec]],
    ...
    let mut add_fields = vec![stats_field.clone()];
    if include_partition_values {
        add_fields.push(pv_field.clone());
    }
    let add_field = Arc::new(Field::new(
        "add",
        ArrowDataType::Struct(Fields::from(add_fields)),
        true,
    ));
    let schema = Arc::new(ArrowSchema::new(vec![add_field]));
    ...
    for group in groups {
        ...
        let mut add_columns: Vec<(Arc<Field>, Arc<dyn Array>)> =
            vec![(stats_field.clone(), Arc::new(stats_struct))];
        if include_partition_values {
            add_columns.push((pv_field.clone(), Arc::new(pv_struct)));
        }
        let add_struct = StructArray::from(add_columns);
        let batch = RecordBatch::try_new(schema.clone(), vec![Arc::new(add_struct)]).unwrap();
        writer.write(&batch).unwrap();
        writer.flush().unwrap();
    }
...
fn surviving_ids(
    ...
    impl<'a> ExpressionTransform<'a> for PrefixAdd {
        transform_output_type!(|'a, T| Cow<'a, T>);

        fn transform_expr_column(&mut self, name: &'a ColumnName) -> Cow<'a, ColumnName> {
            Cow::Owned(ColumnName::new(["add"]).join(name))
        }
    }

    let meta = as_checkpoint_skipping_predicate(pred, &partitions, &HashSet::new(), &stats)
        .map(|predicate| PrefixAdd.transform_pred(&predicate).into_owned());
...
/// Row-group pruning across predicate shapes through the production rewrite and reader.
#[rstest]
...
// All partition values are present, so IS NULL prunes every group and IS NOT NULL keeps them all.
...
/// Null partition leaves (non-Add rows or null-valued Adds) are pruned by comparisons and `IS NOT
/// NULL`, but kept by `IS NULL`.
#[rstest]
...
// Footer min/max ignore the null sibling, so the non-matching range ["a","a"] prunes the group.
#[case::mixed_group_with_null_sibling_pruned(
    &[&[rg(1, 0, 10, Some("a")), rg(2, 100, 110, None)] as &[RgSpec]],
    Predicate::eq(column_name!("part"), Scalar::from("z")),
    vec![],
)]
// A present all-null leaf cannot satisfy a value comparison, so the group is pruned.
...
#[case::all_null_group_pruned_under_is_not_null(
    &[&[rg(1, 0, 10, Some("a"))] as &[RgSpec], &[rg(2, 100, 110, None), rg(3, 200, 210, None)]],
    Predicate::is_not_null(column_name!("part")),
    vec![1],
)]
fn reader_handles_null_partition_groups(
    #[case] groups: &[&[RgSpec]],
    #[case] pred: Predicate,
...
/// A physically absent partition leaf is unknown, unlike a present all-null leaf. Older
/// checkpoints without `partitionValues_parsed` must therefore remain non-pruning.
#[rstest]
#[case::comparison(Predicate::eq(column_name!("part"), Scalar::from("z")))]
#[case::is_not_null(Predicate::is_not_null(column_name!("part")))]
fn reader_keeps_row_group_when_partition_column_is_missing(#[case] pred: Predicate) {
    let tmp =
        write_checkpoint_with_partition_values(&[&[rg(1, 0, 10, Some("a"))] as &[RgSpec]], false);
    assert_eq!(surviving_ids(&tmp, &pred, &["part"], &["x"]), vec![1]);
}
