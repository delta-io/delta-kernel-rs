    ...
    fn get_parquet_schema(&self, file: &FileMeta) -> DeltaResult<SchemaRef> {
        ...
        self.task_executor
            .block_on(async {
                ...
                if file.location.is_presigned() {
                    // For presigned URLs, fetch the file and read metadata
                    let url = file.location.as_str();
                    ...
                    let metadata = ArrowReaderMetadata::load(&bytes.as_ref(), ArrowReaderOptions::default())
                    ...
                    let metadata = ArrowReaderMetadata::load_async(&reader, ArrowReaderOptions::default())
                        .await
                        .map_err(|e| Error::generic(format!("Failed to load parquet metadata: {}", e)))?;
            ...
            .map_err(|e| Error::generic(format!("Task executor error: {}", e)))?
    }
}
