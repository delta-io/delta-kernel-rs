...
impl StatisticsCollector {
    ...
    fn compute_null_counts(column: &ArrayRef, num_rows: usize) -> ArrayRef {
            ...
            DataType::Struct(fields) => {
                ...
                let children: Vec<ArrayRef> = fields
                    ...
                    .map(|(i, field)| {
                        ...
                        let child_counts = Self::compute_null_counts(child_col, num_rows);
    ...
    fn add_to_null_count(null_count: &ArrayRef, add: i64, data_type: &DataType) -> ArrayRef {
        ...
        match data_type {
            DataType::Struct(fields) => {
                let struct_arr = null_count.as_any().downcast_ref::<StructArray>().unwrap();
                let children: Vec<ArrayRef> = fields
                    .iter()
                    .enumerate()
                    .map(|(i, field)| {
            ...
            _ => {
                let arr = null_count.as_any().downcast_ref::<Int64Array>().unwrap();
    ...
    fn merge_null_counts(current: &ArrayRef, new_val: &ArrayRef, data_type: &DataType) -> ArrayRef {
        match data_type {
            DataType::Struct(fields) => {
                let struct_a = current.as_any().downcast_ref::<StructArray>().unwrap();
                let struct_b = new_val.as_any().downcast_ref::<StructArray>().unwrap();
                let children: Vec<ArrayRef> = fields
                    .iter()
                    .enumerate()
                    .map(|(i, field)| {
            ...
            _ => {
                let arr_a = current.as_any().downcast_ref::<Int64Array>().unwrap();
                let arr_b = new_val.as_any().downcast_ref::<Int64Array>().unwrap();
    ...
    fn update(&mut self, batch: &RecordBatch) -> DeltaResult<()> {
        ...
        for i in 0..batch.num_columns() {
            ...
            let batch_null_counts = Self::compute_null_counts(column, batch.num_rows());
            ...
            if column.null_count() < column.len() && column.len() > 0 {
    ...
    fn compare_and_select(
        ...
        match data_type {
            ...
            DataType::Utf8 => {
                let arr_a = a.as_any().downcast_ref::<StringArray>().unwrap();
                let arr_b = b.as_any().downcast_ref::<StringArray>().unwrap();
                let val_a = arr_a.value(0);
                let val_b = arr_b.value(0);
                if select_min {
                    if val_a <= val_b {
            ...
            DataType::Struct(fields) => {
                ...
                let struct_a = a.as_any().downcast_ref::<StructArray>().unwrap();
                let struct_b = b.as_any().downcast_ref::<StructArray>().unwrap();
