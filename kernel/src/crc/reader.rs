...
mod tests {
    ...
    fn test_read_crc_file() {
        ...
        // Get the path to the test CRC file
        let path =
            std::fs::canonicalize(PathBuf::from("./tests/data/crc-full/_delta_log/")).unwrap();
        let url = url::Url::from_directory_path(path).unwrap();
        let crc_url = url.join("00000000000000000000.crc").unwrap();
        let file_meta = FileMeta::new(crc_url, 0, 0);
        let crc_path = ParsedLogPath::try_from(file_meta).unwrap().unwrap();
