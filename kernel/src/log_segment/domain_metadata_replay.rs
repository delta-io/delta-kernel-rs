...
impl LogSegment {
    ...
    async fn visit_domain_metadatas(
        ...
        let mut batches = GeneratorState::Start(self.read_domain_metadata_batches(channel).await?);
        while let Some(actions) = batches.next(channel).await? {
    ...
    fn read_domain_metadata_batches_with_engine(
        ...
        let actions = connector
            .run(async move |channel| log_segment.read_domain_metadata_batches(&channel).await)?;
