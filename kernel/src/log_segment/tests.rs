...
#[tokio::test]
async fn test_create_checkpoint_stream_with_fallback_no_fallback_schema() -> DeltaResult<()> {
    let (store, log_root) = new_in_memory_store();
    let engine = DefaultEngine::new(store.clone());
    let read_schema = get_commit_schema().project(&[ADD_NAME])?;

    let checkpoint_file = format!("{}00000000000000000001.checkpoint.parquet", log_root);
    add_checkpoint_to_store(
        &store,
        add_batch_simple(get_commit_schema().clone()),
        "00000000000000000001.checkpoint.parquet",
    ...
    let log_segment = LogSegment::try_new(
        ListedLogFiles::try_new(
            vec![],
            vec![],
            vec![create_log_path(&checkpoint_file)],
            None,
            None,
        )?,
        log_root,
    ...
    // Test with no fallback schema - should return data without probing
    let mut iter = log_segment.create_checkpoint_stream_with_fallback(
        &engine,
        read_schema.clone(),
        None,
        None,
...
#[tokio::test]
async fn test_create_checkpoint_stream_with_fallback_primary_succeeds() -> DeltaResult<()> {
    let (store, log_root) = new_in_memory_store();
    let engine = DefaultEngine::new(store.clone());
    let read_schema = get_commit_schema().project(&[ADD_NAME])?;
    ...
    // Test with fallback schema provided, but primary succeeds - should return data from primary
    let fallback_schema = read_schema.clone();
    let mut iter = log_segment.create_checkpoint_stream_with_fallback(
        &engine,
        read_schema.clone(),
...
#[tokio::test]
async fn test_create_checkpoint_stream_with_fallback_uses_fallback_on_error() -> DeltaResult<()> {
    use crate::schema::{DataType, StructField, StructType};

    let (store, log_root) = new_in_memory_store();
    let engine = DefaultEngine::new(store.clone());

    // The checkpoint is written with the standard schema (add.path is STRING)
    let checkpoint_file = format!("{}00000000000000000001.checkpoint.parquet", log_root);
    add_checkpoint_to_store(
        &store,
        add_batch_simple(get_commit_schema().clone()),
    ...
    // Create an incompatible primary schema where add.path is LONG instead of STRING
    let incompatible_add_schema = StructType::new_unchecked([
        StructField::nullable("path", DataType::LONG), // Wrong type - should be STRING
    ]);
    let incompatible_schema: crate::schema::SchemaRef =
        Arc::new(StructType::new_unchecked([StructField::nullable(
            ADD_NAME,
            incompatible_add_schema,
        )]));

    // The correct fallback schema
    let fallback_schema = get_commit_schema().project(&[ADD_NAME])?;

    // Reading with incompatible schema should fail, then fallback should succeed
    let mut iter = log_segment.create_checkpoint_stream_with_fallback(
        &engine,
        incompatible_schema,
        Some(fallback_schema),
    ...
    // Should successfully read using the fallback schema
    let ActionsBatch {
        actions,
        is_log_batch,
    } = iter.next().unwrap()?;
