...
/// A multi-part V1 checkpoint is identified by `(version, numParts)`. The hint applies only when
/// its part count matches the checkpoint the segment selected; a different `parts` (a same-version
/// checkpoint with a different part count) must be ignored.
#[rstest]
...
/// The `v2Checkpoint` hint parses from V2 checkpoint tables to its exact contents. Pins, per table,
/// the checkpoint version, file path, sidecar paths, metadata id, created time, and configuration;
/// and -- shared across these fixtures -- a `(3, 7)` protocol with V2Checkpoint/AppendOnly/
/// Invariants features and an unpartitioned parquet `id: long` schema. The non-file actions are
/// exactly protocol + metadata + checkpointMetadata (no txn, no domainMetadata). Also checks the
/// identity gate exposes a matched hint's sidecars but suppresses a mismatched one
/// (`v2-classic-checkpoint-parquet`, whose hint names a UUID checkpoint while the segment selects
...
async fn test_try_new_with_checkpoint(
    ...
    assert!(result.last_checkpoint_hint().is_none());
    assert_eq!(
