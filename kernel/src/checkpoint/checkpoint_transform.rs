...
/// Builds the checkpoint output schema and transform expression together from the read schema.
///
/// The read schema contains every field needed by the transform. This helper applies the same
/// sparse patch to produce both the output schema and the expression patch, keeping the two
