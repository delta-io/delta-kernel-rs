...
//! When typed metadata is requested, also reports whether the checkpoint has compatible parsed
//! stats and partition values.
...
/// A snapshot's resolved checkpoint type and compatible typed metadata schemas.
#[derive(Clone, Debug, PartialEq)]
...
impl CheckpointShape {
    /// Resolve `snapshot`'s checkpoint shape and compatible typed metadata fields.
    ...
    fn from_v2_checkpoint_hint(
        ...
        match segment.checkpoint_hint_sidecars().map(Vec::as_slice) {
            ...
            Some([]) => {
                // A parquet leaf's typed metadata lives in its own schema; read it only when typed
                // metadata was requested. A JSON leaf has no readable schema.
    ...
    /// Build the shape for a manifest checkpoint. Its file actions and typed metadata live in the
    /// sidecars. Use the `_last_checkpoint` sidecar schema when present; otherwise read one
    /// sidecar footer when typed metadata was requested. All sidecars share one schema.
    fn try_new_manifest(
        exec: &dyn PlanExecutor,
        sidecar: FileMeta,
        stats_schema: Option<&SchemaRef>,
        ...
        let sidecar_schema = match hint_sidecar_schema {
            Some(schema) => Some(Arc::new(schema)),
            None if stats_schema.is_some() || partition_schema.is_some() => {
                Some(exec.read_parquet_footer(sidecar)?.schema)
            }
            None => None,
        };
        let parsed_stats_schema = stats_schema.filter(|stats_schema| {
            sidecar_schema.as_ref().is_some_and(|sidecar_schema| {
                LogSegment::schema_has_compatible_stats_parsed(sidecar_schema, stats_schema)
            })
        });
...
mod tests {
    ...
    fn checkpoint_schema_with_partitions(compatible: bool) -> StructType {
        let partitions = if compatible {
            schema! { nullable "part": TIMESTAMP }
        } else {
            schema! { nullable "part": STRING }
        };
        schema! {
            nullable ADD_NAME: {
                nullable "partitionValues_parsed": (partitions),
            }
        }
    }

    #[rstest]
    #[case::compatible_leaf(CheckpointType::Leaf, true)]
    #[case::incompatible_leaf(CheckpointType::Leaf, false)]
    #[case::compatible_manifest(CheckpointType::Manifest, true)]
    #[case::incompatible_manifest(CheckpointType::Manifest, false)]
    fn resolves_compatible_partition_values_schema(
        #[case] checkpoint_type: CheckpointType,
        #[case] compatible: bool,
    ) {
        let exec = CountingExecutor::new();
        let partition_schema = schema_ref! { nullable "part": TIMESTAMP };
        let checkpoint_schema = checkpoint_schema_with_partitions(compatible);
        let shape = match checkpoint_type {
            CheckpointType::Leaf => CheckpointShape::try_new_leaf(
                Some(Arc::new(checkpoint_schema)),
                None,
                Some(&partition_schema),
            ),
            CheckpointType::Manifest => CheckpointShape::try_new_manifest(
                &exec,
                FileMeta::new(Url::parse("memory:///sidecar.parquet").unwrap(), 0, 0),
                None,
                Some(&partition_schema),
                Some(checkpoint_schema),
            )
            .unwrap(),
            CheckpointType::None => unreachable!(),
        };

        assert_eq!(shape.checkpoint_type, checkpoint_type);
        assert_eq!(
            shape.parsed_partition_values_schema.as_ref(),
            compatible.then_some(&partition_schema)
        );
        assert_eq!(exec.footer_reads.load(Ordering::Relaxed), 0);
    }
    ...
    #[test]
    fn partition_only_request_triggers_manifest_footer_read() {
        let (_engine, snapshot, _tempdir) =
            load_test_table("v2-checkpoints-parquet-with-sidecars").unwrap();
        let exec = CountingExecutor::new();
        let partition_schema = schema_ref! { nullable "part": TIMESTAMP };
        ...
        let shape =
            CheckpointShape::try_new(&exec, snapshot.as_ref(), None, Some(&partition_schema))
                .unwrap();

        assert_eq!(shape.checkpoint_type, CheckpointType::Manifest);
        assert!(shape.parsed_partition_values_schema.is_none());
        assert_eq!(exec.query_scans.load(Ordering::Relaxed), 0);
        assert_eq!(exec.footer_reads.load(Ordering::Relaxed), 1);
    }
