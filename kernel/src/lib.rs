...
pub use error::{
    BoxedError, DeltaError, DeltaErrorCode, DeltaErrorParameter, DeltaResult, DeltaResultIterator,
    DeltaResultIteratorStatic, EngineError, EngineErrorKind, EngineResult, EngineResultIterator,
    EngineResultIteratorStatic, Error,
};
...
pub trait JsonHandler: AsAny {
    ...
    /// additionally polls the token at action-batch boundaries. An engine that overrides this
    /// may assume kernel has already performed the pre-read check, and should focus on
...
pub trait ParquetHandler: AsAny {
    ...
    /// overrides this may assume kernel has already performed the pre-read check, and should
    /// focus on interrupting its in-flight I/O.
...
pub trait Engine: AsAny {
    ...
    /// # Errors
    ///
    /// Returns an error when [`plan_executor`](Self::plan_executor) is `None`.
    #[cfg(feature = "declarative-plans")]
