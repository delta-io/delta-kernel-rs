...
pub trait JsonHandler: AsAny {
    ...
    /// Note: The [`FileDataReadResultIterator`] must emit data from files in
    /// the order that `files` is given. For example if files ["a", "b"] is
    /// provided, then the engine data iterator must first return all the engine
    /// data from file "a", _then_ all the engine data from file "b". Moreover, for
    /// a given file, all of its [`EngineData`] and constituent rows must be in order
    /// that they occur in the file. Consider a file with rows 1, 2, 3. The following
    /// are legal iterator batches:
