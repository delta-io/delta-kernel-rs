...
/// - If the field already has column mapping metadata, it is preserved and `max_id` is
///   updated to track the highest ID seen.
/// - If the field lacks metadata, a new unique ID and physical name are assigned.
///
/// # Arguments
///
...
fn assign_field_column_mapping(field: &StructField, max_id: &mut i64) -> DeltaResult<StructField> {
    ...
    // For CREATE TABLE, we don't support schemas with pre-existing column mapping metadata.
    // This avoids conflicts between user-provided IDs and the IDs we assign.
    // ALTER TABLE (adding columns) will need different handling in the future.
    if has_id {
        return Err(Error::generic(format!(
            "Field '{}' already has column mapping ID metadata. \
...
///
/// When column mapping is enabled (mode = Name or Id), returns the physical name from the
/// field's metadata. When column mapping is disabled (mode = None), returns the logical name.
///
/// This function only handles top-level columns, which is appropriate for clustering columns
/// since they are required to be top-level by the Delta protocol.
///
/// # Arguments
///
/// * `logical_name` - The logical column name
/// * `schema` - The schema containing column mapping metadata
/// * `mode` - The column mapping mode
///
/// # Returns
///
/// The physical column name, or an error if the column is not found.
pub(crate) fn get_physical_clustering_column_name(
    logical_name: &str,
    schema: &StructType,
    mode: ColumnMappingMode,
) -> DeltaResult<String> {
    ...
    Ok(get_physical_name(field).to_string())
}
...
mod tests {
    ...
    fn test_get_column_mapping_mode_from_properties() {
        // No property set - returns None mode
        ...
        properties.insert(COLUMN_MAPPING_MODE_KEY.to_string(), "name".to_string());
        assert_eq!(
            get_column_mapping_mode_from_properties(&properties).unwrap(),
        ...
        // Mode = id
        let mut properties = HashMap::new();
        properties.insert(COLUMN_MAPPING_MODE_KEY.to_string(), "id".to_string());
        assert_eq!(
            get_column_mapping_mode_from_properties(&properties).unwrap(),
            ColumnMappingMode::Id
        );
        ...
        // Mode = none (explicit)
        let mut properties = HashMap::new();
        properties.insert(COLUMN_MAPPING_MODE_KEY.to_string(), "none".to_string());
        assert_eq!(
            get_column_mapping_mode_from_properties(&properties).unwrap(),
            ColumnMappingMode::None
        );
        ...
        // Invalid mode
        let mut properties = HashMap::new();
        properties.insert(COLUMN_MAPPING_MODE_KEY.to_string(), "invalid".to_string());
        let result = get_column_mapping_mode_from_properties(&properties);
    ...
    fn test_assign_column_mapping_metadata_rejects_existing_id() {
        ...
        // Schema with pre-existing column mapping ID should be rejected
        let schema = StructType::new_unchecked([
        ...
        assert!(
            err_msg.contains("already has column mapping ID metadata"),
            "Expected error about existing column mapping ID, got: {}",
            err_msg
        );
    }
    ...
    /// Helper to verify a struct field has column mapping metadata
    fn assert_has_column_mapping_metadata(field: &StructField) {
        assert!(
    ...
    fn test_assign_column_mapping_metadata_map_with_struct_key_and_value() {
        ...
        assert_has_column_mapping_metadata(map_field);
        ...
        if let DataType::Map(inner_map) = &map_field.data_type {
            ...
            assert_has_column_mapping_metadata(field_k);
            ...
            assert_has_column_mapping_metadata(field_v);
        } else {
    ...
    fn test_assign_column_mapping_metadata_array_with_struct_element() {
        ...
        assert_has_column_mapping_metadata(array_field);
        ...
        if let DataType::Array(inner_array) = &array_field.data_type {
            ...
            assert_has_column_mapping_metadata(field_elem);
        } else {
    ...
    fn test_assign_column_mapping_metadata_double_nested_array() {
        ...
        assert_has_column_mapping_metadata(outer_field);

        // Navigate: array -> array -> struct -> field
        if let DataType::Array(outer) = &outer_field.data_type {
            if let DataType::Array(inner) = outer.element_type() {
                let deep_struct = unwrap_struct(inner.element_type(), "inner array element");
                let field_deep = deep_struct.field("deep").unwrap();
                assert_has_column_mapping_metadata(field_deep);
            } else {
    ...
    #[test]
    fn test_assign_column_mapping_metadata_awful_nesting() {
        // Test: array<map<array<struct<k: int>>, array<struct<v: int>>>>
        // This is the "cursed" case with awful map/array nesting
        use crate::schema::DataType;
        ...
        assert_has_column_mapping_metadata(cursed_field);

        // Navigate: array -> map -> key array -> struct -> field
        //                        -> value array -> struct -> field
        if let DataType::Array(outer) = &cursed_field.data_type {
            if let DataType::Map(inner_map) = outer.element_type() {
                // Check key path: array<struct<k>>
                if let DataType::Array(key_arr) = inner_map.key_type() {
                ...
                if let DataType::Array(val_arr) = inner_map.value_type() {
    ...
    #[test]
    fn test_get_physical_clustering_column_name_no_mapping() {
        use crate::schema::DataType;
        ...
        let result =
            get_physical_clustering_column_name("a", &schema, ColumnMappingMode::None).unwrap();
    ...
    #[test]
    fn test_get_physical_clustering_column_name_with_mapping() {
        use crate::schema::DataType;
        ...
        let result =
            get_physical_clustering_column_name("a", &schema, ColumnMappingMode::Name).unwrap();
    ...
    #[test]
    fn test_get_physical_clustering_column_name_not_found() {
        use crate::schema::DataType;

        let schema = StructType::new_unchecked([StructField::new("a", DataType::INTEGER, false)]);

        let result =
            get_physical_clustering_column_name("nonexistent", &schema, ColumnMappingMode::Name);
