...
impl StructField {
    ...
    ///
    /// The physical name depends on the column mapping mode:
    /// - `None`: Always returns the logical name
    /// - `Id` or `Name`: Returns the physical name from metadata if present, otherwise logical name
    ///
