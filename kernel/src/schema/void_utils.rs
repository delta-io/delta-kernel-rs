...
/// Adds void-field-stripping operations to the given StructPatch, recursing into nested structs.
/// Returns the input patch unchanged when the schema contains no void.
...
fn add_void_stripping_inner(
    mut patch: ExpressionStructPatch,
    st: &StructType,
    path: &[&str],
) -> (ExpressionStructPatch, bool) {
