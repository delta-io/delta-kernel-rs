...
impl Snapshot {
    ...
    /// - `Ok(None)` - ICT is not enabled, or version predates ICT enablement
...
mod tests {
    ...
    async fn test_get_timestamp_fails_when_commit_missing() -> DeltaResult<()> {
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("Last commit file not found in log segment"));
    ...
    async fn test_get_timestamp_with_checkpoint_and_commit_same_version() -> DeltaResult<()> {
        ...
        // This verifies Phase 1 fix where we fall back to reading from storage when commit is not in log segment.
        ...
        // With Phase 1 fix, we should successfully read ICT by falling back to storage
        let timestamp = snapshot.get_in_commit_timestamp(&engine)?;
