//! Step 2: Hive-style partition path encoding.
//!
...
//!
//! ```text
//! Step 1 (serialization):  Scalar::String("US/East")  -->  "US/East"     (partitionValues)
//! Step 2 (THIS MODULE):    "US/East"                  -->  "US%2FEast"   (directory name)
//! ```
//!
//! # Encoding rules
//!
...
pub fn build_partition_path(columns: &[(&str, Option<&str>)]) -> String {
    ...
    let cap: usize = columns
        .iter()
        .map(|(n, v)| n.len() + 1 + v.map_or(HIVE_DEFAULT_PARTITION.len(), str::len) + 1)
        .sum();
