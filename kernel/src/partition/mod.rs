...
//!         |
//!         |  Step 1: serialization  -->  "US/East"    (partitionValues string)
//!         |  Step 2: path encoding  -->  "US%2FEast"  (Hive-style directory name)
//!         |  Step 3: validation     -->  (type/key checks)
//!         v
//!   AddFile { partitionValues: {"region": "US/East"}, path: "region=US%2FEast/..." }
//! ```
//!
//! - **Step 1: Serialization** (FUTURE: `partition/serialization.rs`): converts typed
//!   `Scalar` values to protocol-compliant strings for the `partitionValues` map in Add
//!   actions. Null, empty string, and empty binary all serialize as JSON `null`.
//!
//! - **Step 2: Path encoding** ([`hive`] module): percent-encodes the serialized string
//!   for use in Hive-style filesystem directory paths. Only a small set of ASCII characters
//!   is encoded; spaces and non-ASCII pass through raw. Hive-style paths are optional; the
//!   Delta protocol does not require them. Kernel uses them for partitioned tables without
//!   column mapping enabled.
//!
//! - **Step 3: Validation** (FUTURE: `partition/validation.rs`): validates partition
//!   column names and value types against the table schema.
//!
//! There is also a separate encoding applied by the writer (e.g., Delta-Spark) when
//! constructing `add.path` in the Delta log. This is a URI encoding per RFC 2396, more
//! aggressive than step 2. Characters like space (0x20), `<`, `>`, `{`, `}`, and `|`
//! that survive Hive encoding get percent-encoded in `add.path`.
//!
//! # Encoding by type
//!
...
//! 50  | X'DEADBEEF'              | p=<raw high bytes>             | same                         | same (corrupts!)
...
//!   valid UTF-8, they work (e.g., `X'48454C4C4F'` = "HELLO"). High bytes like
//!   `X'DEADBEEF'` produce corrupted paths.
