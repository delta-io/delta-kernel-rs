...
impl MapToStructOptions {
    /// Interpret space-separated `TIMESTAMP` values in `timestamp_timezone`.
    ///
...
/// - TIMESTAMP: parse a protocol-formatted space-separated timestamp in the timezone from
///   [`MapToStructOptions`], or UTC by default. Parse a protocol-formatted ISO 8601 timestamp with
///   an explicit offset as an absolute instant. Truncate fractional seconds to microseconds.
...
///
/// The configured timezone does not affect a timestamp carrying an explicit offset. The
/// protocol's UTC-adjusted ISO 8601 form therefore reads independently of the configured timezone.
///
