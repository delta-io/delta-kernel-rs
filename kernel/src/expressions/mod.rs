...
impl MapToStructOptions {
    ...
    /// Returns the configured timezone, or `None` when the default UTC interpretation applies.
...
/// Transforms a `Map<String, String>` column into a struct of primitive fields whose schema is
/// provided by the evaluator's output type (via `result_type`). Each row in the map column becomes
/// one row in the output struct column: a `key` -> `value` mapping in the map means the struct
/// field named `key` receives `value`, parsed into the field's target type. Offset-less `TIMESTAMP`
/// values use the timezone in [`MapToStructOptions`], or UTC when no timezone is configured. An
/// empty-string value is the exception (aligning with Spark): it casts to itself for string, to
...
impl Expression {
    ...
    /// Creates a map-to-struct expression. See [`MapToStructExpression`] for its contract.
