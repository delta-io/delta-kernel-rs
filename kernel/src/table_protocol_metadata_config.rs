...
impl TableProtocolMetadataConfig {
    ...
    /// Return a new config with updated partition columns.
...
mod tests {
    ...
    fn test_try_from_creates_bare_protocol() {
        ...
        let config =
            TableProtocolMetadataConfig::new(test_schema(), vec![], properties).unwrap();
    ...
    fn test_try_from_passes_all_properties_through() {
        ...
        assert_eq!(
            config.metadata.configuration().get("delta.minReaderVersion"),
            Some(&"3".to_string())
        );
    }
    ...
    fn test_apply_transforms_with_no_signals() {
        ...
        let config =
            TableProtocolMetadataConfig::new(test_schema(), vec![], properties.clone())
                .unwrap();
