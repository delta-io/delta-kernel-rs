...
mod tests {
    ...
    #[tokio::test]
    async fn sequential_reads_commit_with_non_string_operation_parameters() -> DeltaResult<()> {
        ...
        // Create a commit file with metadata, protocol, add, and commitInfo with non-string operationParameters
        // The commitInfo has boolean (false) and numeric (5) values in operationParameters
        // This simulates what DBR writes and caused the reported issue
        let commit_content = r#"{"commitInfo":{"timestamp":1768528207883,"operation":"CREATE TABLE AS SELECT","operationParameters":{"partitionBy":"[]","clusterBy":"[]","description":null,"isManaged":"true","properties":"{}","statsOnLoad":false},"isolationLevel":"WriteSerializable","isBlindAppend":true,"operationMetrics":{"numFiles":"1","numOutputRows":"1","numOutputBytes":"496"},"tags":{"noRowsCopied":"true","restoresDeletedRows":"false"},"engineInfo":"Databricks-Runtime/<unknown>","txnId":"6c3951b6-66d8-4a3d-be29-6cafcd7378f1"}}
{"metaData":{"id":"e6452640-5b76-4cff-89e7-8d9f6a592c2f","format":{"provider":"parquet","options":{}},"schemaString":"{\"type\":\"struct\",\"fields\":[{\"name\":\"id\",\"type\":\"long\",\"nullable\":true,\"metadata\":{}}]}","partitionColumns":[],"configuration":{},"createdTime":1768528205133}}
{"protocol":{"minReaderVersion":1,"minWriterVersion":2}}
{"add":{"path":"part-00000-61d940c8-6985-4c9f-87cd-8e04faa20ca8-c000.snappy.parquet","partitionValues":{},"size":496,"modificationTime":1768528207522,"dataChange":true,"stats":"{\"numRecords\":1,\"minValues\":{\"id\":0},\"maxValues\":{\"id\":0},\"nullCount\":{\"id\":0}}","tags":{"INSERTION_TIME":"1768528207522000","MIN_INSERTION_TIME":"1768528207522000","MAX_INSERTION_TIME":"1768528207522000","OPTIMIZE_TARGET_SIZE":"268435456"}}}
"#;

        // Write to in-memory store
        let commit_0_path = Path::from("_delta_log/00000000000000000000.json");
        store
            .put(&commit_0_path, commit_content.into())
            .await
            .unwrap();

        let table_root = url::Url::parse("memory:///").unwrap();

        // Build snapshot - this should not fail with "expected string got false"
        let snapshot = Snapshot::builder_for(table_root).build(engine.as_ref())?;
        let log_segment = snapshot.log_segment();

        // Create sequential phase processor
        let state_info = Arc::new(StateInfo::try_new(
            snapshot.schema(),
            snapshot.table_configuration(),
            None,
        ...
        // Verify we processed at least one batch
        let mut processed = 0;
        for batch in sequential.by_ref() {
            assert!(batch.is_ok());
            processed += 1;
