...
/// frozen log segment.
// TODO: Use Arc<Snapshot> to avoid deep-cloning snapshots.
#[derive(Clone)]
pub struct Snapshot {
    span: tracing::Span,
...
impl Snapshot {
    ...
    pub fn get_app_id_version(
        ...
        drive_workflow(engine, self.start_get_app_id_version(application_id))
    }

    /// Start a connector-driven workflow that fetches the latest transaction version for an
    /// application id.
    ///
    ...
    pub fn start_get_app_id_version(
        &self,
        application_id: &str,
    ) -> DeltaResult<Workflow<Option<i64>>> {
    ...
    pub fn get_domain_metadata(
        ...
        drive_workflow(engine, self.start_get_domain_metadata(domain))
    }

    /// Start a connector-driven workflow that fetches user-controlled domain metadata.
    ///
    /// Returns an error if `domain` is system-controlled. Errors after suspension are returned by
    /// the corresponding resume handle.
    pub fn start_get_domain_metadata(&self, domain: &str) -> DeltaResult<Workflow<Option<String>>> {
    ...
    pub(crate) fn get_clustering_column_infos_with_engine(
        ...
        // TODO: Use Arc<Snapshot> to avoid this deep clone.
        let snapshot = self.clone();
        let workflow = Workflow::start(async move |channel| {
    ...
    pub(crate) fn get_domain_metadatas_internal_with_engine(
        ...
        drive_workflow(engine, self.start_get_domain_metadatas_internal(domains))
    }

    /// Start a connector-driven workflow that loads the requested domain metadata.
    ///
    /// Returns the first workflow state. Errors after suspension are returned by the corresponding
    /// resume handle.
    #[internal_api]
    pub(crate) fn start_get_domain_metadatas_internal(
        &self,
        domains: Option<&HashSet<&str>>,
    ) -> DeltaResult<Workflow<DomainMetadataMap>> {
        // TODO: Use Arc<Snapshot> to avoid this deep clone.
        let snapshot = self.clone();
    ...
    pub(crate) fn get_domain_metadata_internal_with_engine(
        ...
        drive_workflow(engine, self.start_get_domain_metadata_internal(domain))
    }

    /// Start a connector-driven workflow that fetches user-controlled or system-controlled domain
    /// metadata.
    ///
    /// Returns the first workflow state. Errors after suspension are returned by the corresponding
    /// resume handle.
    #[internal_api]
    pub(crate) fn start_get_domain_metadata_internal(
        &self,
        domain: &str,
    ) -> DeltaResult<Workflow<Option<String>>> {
        // TODO: Use Arc<Snapshot> to avoid this deep clone.
        let snapshot = self.clone();
        let domain = domain.to_owned();
        Workflow::start(async move |channel| {
    ...
    pub(crate) fn get_in_commit_timestamp_with_engine(
        ...
        drive_workflow(engine, self.start_get_in_commit_timestamp())
    }

    /// Start a connector-driven workflow that gets this snapshot's in-commit timestamp.
    ///
    /// Returns the first workflow state. Errors after suspension are returned by the corresponding
    /// resume handle.
    #[internal_api]
    pub(crate) fn start_get_in_commit_timestamp(&self) -> DeltaResult<Workflow<Option<i64>>> {
        // TODO: Use Arc<Snapshot> to avoid this deep clone.
        let snapshot = self.clone();
        Workflow::start(async move |channel| snapshot.get_in_commit_timestamp(&channel).await)
    }
    ...
    pub fn transaction(self: Arc<Self>, engine: &dyn Engine) -> DeltaResult<Transaction> {
        drive_workflow(engine, self.start_transaction())
    }

    /// Start a connector-driven workflow that creates a transaction for this snapshot.
    ///
    /// Returns the first workflow state. Errors after suspension are returned by the corresponding
    /// resume handle.
    pub fn start_transaction(self: Arc<Self>) -> DeltaResult<Workflow<Transaction>> {
        Workflow::start(async move |channel| {
    ...
    pub fn write_checksum(
        ...
        drive_workflow(engine, self.start_write_checksum())
    }
    ...
    pub fn start_write_checksum(
        ...
        Workflow::start(async move |channel| snapshot.write_checksum_impl(&channel).await)
    }
    ...
    pub fn publish(
        ...
        EngineConnector::new(engine).drive_with_committer(
            self.start_publish(committer.is_catalog_committer()),
            engine,
            committer,
        )
    }

    /// Start a connector-driven workflow that publishes this snapshot's catalog commits.
    ///
    ...
    ) -> DeltaResult<Workflow<SnapshotRef>> {
        let snapshot = Arc::clone(self);
        Workflow::start(async move |channel| {
...
mod tests {
    ...
    use crate::coroutine::{Cursor, Page, PageRequest, Request};
        ...
        let timestamp = loop {
            workflow = match workflow {
                ...
                Workflow::Request(Request::ReadJson(PageRequest::Start(_, resume))) => {
                    resume(Ok(Page::new(Vec::new(), Some(Cursor::new(1_i64)))))?
                }
                Workflow::Request(Request::ReadJson(PageRequest::Continue(cursor, resume))) => {
                    ...
                    resume(Ok(Page::new(vec![batch], None)))?
                }
