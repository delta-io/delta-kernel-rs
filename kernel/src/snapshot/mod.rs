...
mod tests {
    ...
    async fn test_snapshot_new_from() -> DeltaResult<()> {
        ...
        fn test_new_from(store: Arc<InMemory>) -> DeltaResult<()> {
            ...
            let engine =
                DefaultEngineBuilder::new(PrefixedStore::new(store, Path::from(""))).build();
        ...
        let engine =
            DefaultEngineBuilder::new(PrefixedStore::new(Arc::new(store.fork()), Path::from("")))
                .build();
        let base_snapshot = Snapshot::builder_for(table_root)
            .at_version(0)
            .build(&engine)?;
        ...
        let engine =
            DefaultEngineBuilder::new(PrefixedStore::new(store_3c_i, Path::from(""))).build();
        let base_snapshot = Snapshot::builder_for(table_root)
            .at_version(0)
            .build(&engine)?;
        assert!(matches!(
