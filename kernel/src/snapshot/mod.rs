...
impl Snapshot {
    ...
    /// engine info, blind append, domain metadata, set-transaction, commit info) and
    /// schema-evolution operations (`add_column`, `set_nullable`) before building a
    /// [`Transaction`](crate::transaction::Transaction) via
