...
async fn incompatible_schemas_fail() {
    async fn assert_schema_check(
        commit_schema: StructType,
        cdf_schema: StructType,
        expected: Result<(), SchemaComparisonError>,
    ) {
        let engine = Arc::new(SyncEngine::new());
        let mut mock_table = LocalMockTable::new();
        ...
        match (res, expected) {
            (Ok(_), Ok(_)) => { /* passes */ }
            (
                Err(Error::ChangeDataFeedIncompatibleSchema(_, schema_err)),
                Err(expected_schema_err),
            ) => {
                assert_eq!(schema_err, expected_schema_err);
            }
            (Ok(_), expected) => {
                panic!("Got Ok, but expected {expected:?}");
            }
            (Err(err), expected) => {
                panic!("Got {err:?}, but expected {expected:?}");
            }
        }
    }
    ...
    // CDF schema has an non-nullable column `value2` that is not present in the commit schema.
    // The CDF schema has fields: `id: int`, `value: string`, `value2: string`.
    // This commit has schema with fields: `id: int` and `value: string`.
    let cdf_schema = StructType::new([
        StructField::nullable("id", DataType::INTEGER),
        StructField::nullable("value", DataType::STRING),
        StructField::not_null("value2", DataType::STRING),
    ]);
    let commit_schema = StructType::new([
        StructField::nullable("id", DataType::INTEGER),
        StructField::nullable("value", DataType::STRING),
    ...
    assert_schema_check(schema, get_schema(), Ok(())).await;
