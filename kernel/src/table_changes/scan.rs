...
impl TableChangesScanBuilder {
    ...
    pub fn build(self) -> DeltaResult<TableChangesScan> {
        ...
        let raw_table_schema: SchemaRef = self.table_changes.schema().clone().into();
        // If no projection is supplied, default to the full CDF-extended schema (SELECT *).
        let raw_logical_read_schema = self.schema.unwrap_or_else(|| raw_table_schema.clone());
        ...
        let table_schema = LogicalSchema::new(raw_table_schema, table_config);
...
mod tests {
    ...
    fn cdf_predicate_on_unprojected_data_column_in_table_schema_succeeds() {
        ...
        assert_eq!(scan.logical_schema().raw_schema().fields().len(), 1);
        assert!(matches!(&scan.state_info.physical_predicate,
