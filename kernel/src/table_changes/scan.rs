...
mod tests {
    ...
    fn simple_table_changes_scan_builder() {
        ...
        // Verify _change_type is handled as Dynamic
        assert!(transform_spec.iter().any(|t| matches!(t,
            FieldTransformSpec::DynamicColumn { physical_name, .. }
            if physical_name == "_change_type"
        )));

        // Verify CDF metadata columns are handled
        assert_eq!(
            transform_spec
                .iter()
                .filter(|t| matches!(t, FieldTransformSpec::MetadataDerivedColumn { .. }))
                .count(),
            2
    ...
    fn projected_and_filtered_table_changes_scan_builder() {
        ...
        assert!(matches!(&transform_spec[0],
            FieldTransformSpec::MetadataDerivedColumn { field_index, .. } if *field_index == 1
        ));
