...
impl TableChangesScanBuilder {
    ...
    pub fn build(self) -> DeltaResult<TableChangesScan> {
        ...
        // - If the field is a column generated by CDF, it is ([`ColumnType::Metadata`]).
        ...
        let all_fields = logical_schema
            ...
            .map(|(index, logical_field)| -> DeltaResult<_> {
                ...
                {
                    // CDF Columns are generated, so they do not have a column mapping. They will be
                    // handled separately and used to build an expression when transforming physical
                    // data to logical.
...
fn read_scan_file(
    ...
    if scan_file.scan_type == super::scan_file::CdfScanFileType::Cdc {
        all_fields_converted = all_fields_converted
            .into_iter()
    ...
    let physical_to_logical_expr = super::physical_to_logical::create_unified_transform_expr(
        &scan_file,
