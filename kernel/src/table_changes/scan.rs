...
use crate::scan::{ColumnType, FieldTransformSpec, PhysicalPredicate, Scan, ScanResult};
...
impl TableChangesScanBuilder {
    ...
    pub fn build(self) -> DeltaResult<TableChangesScan> {
        ...
        // - If the field is a CDF-specific metadata column, it is one of [`ColumnType::CdfChangeType`],
        //   [`ColumnType::CdfCommitVersion`], or [`ColumnType::CdfCommitTimestamp`].
            ...
            .map(|(index, logical_field)| -> DeltaResult<_> {
                match logical_field.name() {
                    name if name == super::COMMIT_VERSION_COL_NAME => {
                        Ok(ColumnType::Cdf(crate::scan::CdfCol::CommitVersion, index))
                    }
                    name if name == super::COMMIT_TIMESTAMP_COL_NAME => {
                        Ok(ColumnType::Cdf(crate::scan::CdfCol::CommitTimestamp, index))
                    }
                    name if name == super::CHANGE_TYPE_COL_NAME => Ok(ColumnType::Cdf(
                        crate::scan::CdfCol::ChangeType(logical_field.name().to_string()),
                        index,
                    )),
...
fn read_scan_file(
    ...
    // Get the actual file-level physical schema (which includes _change_type for CDC files)
    let file_physical_schema = scan_file_physical_schema(&scan_file, physical_schema.as_ref());
    ...
    let read_result_iter =
        engine
            .parquet_handler()
            .read_parquet_files(&[file], file_physical_schema, None)?;
...
mod tests {
    ...
    fn simple_table_changes_scan_builder() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                ColumnType::Cdf(
                    crate::scan::CdfCol::ChangeType("_change_type".to_string()),
                    2,
                ),
                ColumnType::Cdf(crate::scan::CdfCol::CommitVersion, 3,),
                ColumnType::Cdf(crate::scan::CdfCol::CommitTimestamp, 4,),
            ]
            .into()
        );
    ...
    fn projected_and_filtered_table_changes_scan_builder() {
        ...
        assert_eq!(
            ...
            vec![
                ColumnType::Selected("id".to_string()),
                ColumnType::Cdf(crate::scan::CdfCol::CommitVersion, 1,),
            ]
            .into()
        );
        assert_eq!(
