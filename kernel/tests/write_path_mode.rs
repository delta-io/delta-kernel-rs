//! Tests verifying that kernel writes relative paths in Delta log `add.path` fields,
//! matching the Delta Spark convention.
...
/// Writes a batch via an unpartitioned transaction and returns the post-commit snapshot.
async fn write_batch_simple(
...
async fn test_write_uses_relative_paths_and_readback() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let snapshot = write_batch_simple(
        &snapshot,
        engine.as_ref(),
    ...
    // Verify the log contains a relative path (no URL scheme like "s3://")
