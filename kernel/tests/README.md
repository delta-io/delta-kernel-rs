...
| `partition_cm/name` | data/ | `value: int, category: string` partitioned by `category` | v3/v7 | r:`columnMapping` w:`columnMapping`, `columnMapping.mode=name` | Partitioned write with CM name mode | `write::column_mapping::test_column_mapping_partitioned_write(cm_name)`, `scan/tests.rs::test_physical_partition_column_names_with_column_mapping` |
