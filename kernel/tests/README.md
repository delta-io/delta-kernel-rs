...
| `v1-struct-stats-schema-evolution` | data/ | `id: long, value: long` (evolved from `value: string`) | v3/v7 | r:`deletionVectors` w:`deletionVectors,appendOnly,invariants`, `writeStatsAsStruct=true`, `writeStatsAsJson=false` | Copied from `v1-single-part-struct-stats-only` with v6 commit that evolves schema (value: string -> long) and adds one file with JSON stats. Tests per-column stats degradation when stats_parsed types mismatch current schema. | `read.rs::stats_parsed_type_mismatch_degrades_to_per_column_skipping` |
