...
| Table | Location | Schema | Protocol (R/W) | Features | Description | Tests |
|-------|----------|--------|----------|----------|-------------|-------|
| `parsed-stats` | data/ | `id: long, name: string, age: long, salary: long, ts_col: timestamp` | v1/v2 | `checkpointInterval=3` | 100 rows per commit (v0-v5). Checkpoint at v3 with `stats_parsed` columns including `ts_col` for testing timestamp stat truncation adjustment and data skipping predicates. | `scan/tests.rs::test_scan_metadata_with_stats_columns/test_scan_metadata_stats_columns_with_predicate/test_prefix_columns_simple/data_skipping_mode_gates_actions_meta_predicate/test_build_actions_meta_predicate_no_predicate/test_build_actions_meta_predicate_static_skip_all/data_skipping_mode_is_independent_of_stats_output/test_default_stats_options_no_struct_output/test_scan_metadata_with_specific_stats_columns`, `scan/data_skipping/tests.rs::timestamp_predicate_skipping/unsupported_predicate_skipping` |
