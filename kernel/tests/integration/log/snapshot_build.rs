...
async fn time_travel_snapshot_is_never_built_as_latest() -> DeltaResult<()> {
    ...
    // Time-traveling to another (older) version is not considered built as latest. This is a
    // fresh build, since the incremental path from a v3 base cannot move backward to v2.
    let another_version = Snapshot::builder_for(&table_path)
        .at_version(2)
        .build(engine.as_ref())?;
    assert_eq!(another_version.version(), 2);
