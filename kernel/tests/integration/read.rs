...
async fn stats() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let batch2 = make_top_level_fields_nullable(&generate_batch(vec![
        ("id", vec![5, 7].into_array_ref()),
        ("val", vec!["e", "g"].into_array_ref()),
    ])?);
...
async fn test_partition_pruning_with_column_mapping(
    ...
    let batch = generate_batch(vec![("phys_val", vec!["x", "y", "z"].into_array_ref())])?;
...
async fn predicate_on_non_nullable_partition_column() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let batch = generate_batch(vec![("val", vec!["a", "b", "c"].into_array_ref())])?;
...
{
    let batch_1 = generate_batch(vec![("val", vec!["a", "b", "c"].into_array_ref())])?;
    let batch_2 = generate_batch(vec![("val", vec!["d", "e", "f"].into_array_ref())])?;
...
async fn test_row_index_metadata_column() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let batch1 = generate_batch(vec![
        ("id", vec![1i32, 2, 3, 4, 5].into_array_ref()),
        ("value", vec!["a", "b", "c", "d", "e"].into_array_ref()),
    ])?;
    let batch2 = generate_batch(vec![
        ("id", vec![10i32, 20, 30].into_array_ref()),
        ("value", vec!["x", "y", "z"].into_array_ref()),
    ])?;
    let batch3 = generate_batch(vec![
        ("id", vec![100i32, 200, 300, 400].into_array_ref()),
        ("value", vec!["p", "q", "r", "s"].into_array_ref()),
    ])?;
...
async fn test_file_path_metadata_column() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let batch1 = generate_batch(vec![
        ("id", vec![1i32, 2, 3].into_array_ref()),
        ("value", vec!["a", "b", "c"].into_array_ref()),
    ])?;
    let batch2 = generate_batch(vec![
        ("id", vec![10i32, 20].into_array_ref()),
        ("value", vec!["x", "y"].into_array_ref()),
    ])?;
...
async fn read_table_with_void_column() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let batch = generate_batch(vec![("id", vec![1, 2, 3].into_array_ref())])?;
...
async fn scan_with_void_schema(
    ...
    let batch = generate_batch(vec![(
        "id",
        (1..=num_rows).collect::<Vec<_>>().into_array_ref(),
    )])?;
...
async fn read_table_with_void_partition_column() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let batch = generate_batch(vec![("id", vec![1, 2].into_array_ref())])?;
