//! Integration tests for writing ANSI interval columns (the `intervalType` feature).
//!
...
/// Writing interval data to a table whose protocol does not declare the `intervalType` feature is
/// refused (the table is not silently upgraded). Holds regardless of the cargo gate, since this is
/// protocol correctness rather than kernel-support gating.
#[tokio::test]
...
mod feature_disabled {
    ...
    /// With the cargo feature off, kernel does not support `intervalType`, so starting a write
    /// transaction on a table that lists the feature is blocked.
    #[tokio::test]
