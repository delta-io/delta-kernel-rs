...
/// Negative: schema with two fields sharing the same preserved id is rejected during
/// table creation (caught by `make_physical` when constructing `TableConfiguration`).
#[test]
fn test_create_table_rejects_duplicate_preserved_ids() {
    let (_temp_dir, table_path, engine) = test_table_setup().unwrap();
