...
async fn test_create_simple_table() -> DeltaResult<()> {
    ...
    let create_txn =
        TableManager::create_table(&table_path, schema.clone(), "DeltaKernel-RS/0.17.0")
            .build(engine.as_ref())?;

    let _txn = create_txn.commit_metadata(engine.as_ref(), Box::new(FileSystemCommitter::new()))?;
...
async fn test_create_table_with_properties() -> DeltaResult<()> {
    ...
    let create_txn = TableManager::create_table(&table_path, schema.clone(), "FinanceApp/2.5.1")
...
async fn test_create_table_empty_schema() -> DeltaResult<()> {
    ...
    // Try to create table with empty schema - should fail
    let result =
        TableManager::create_table(&table_path, schema, "InvalidApp/0.1.0").build(engine.as_ref());
...
async fn test_commit_info_is_written_to_log() -> DeltaResult<()> {
    ...
    // Create table
    let create_txn =
        TableManager::create_table(&table_path, schema, engine_info).build(engine.as_ref())?;

    create_txn.commit_metadata(engine.as_ref(), Box::new(FileSystemCommitter::new()))?;
    ...
    // Verify we have exactly 3 actions: Protocol, Metadata, CommitInfo
    assert_eq!(
        actions.len(),
        3,
        "Expected 3 actions (protocol, metaData, commitInfo), found {}",
        actions.len()
    );

    // Verify Protocol action
    let protocol_action = &actions[0];
    assert!(
        protocol_action.get("protocol").is_some(),
        "First action should be protocol"
    );
    ...
    let metadata_action = &actions[1];
    assert!(
        metadata_action.get("metaData").is_some(),
        "Second action should be metaData"
    );
    ...
    let commit_info_action = &actions[2];
    assert!(
        commit_info_action.get("commitInfo").is_some(),
        "Third action should be commitInfo - THIS WOULD FAIL IF commitInfo WAS NOT WRITTEN!"
    );

    let commit_info = commit_info_action.get("commitInfo").unwrap();

    // Verify CommitInfo fields
    assert!(
    ...
    assert_eq!(
        ...
        "CommitInfo operation should be 'CREATE TABLE'"
    );
...
async fn test_log_action_order() -> DeltaResult<()> {
    ...
    let create_txn =
        TableManager::create_table(&table_path, schema, "OrderTest/1.0").build(engine.as_ref())?;
    create_txn.commit_metadata(engine.as_ref(), Box::new(FileSystemCommitter::new()))?;
    ...
    assert!(
        lines[2].contains("\"commitInfo\""),
        "Third action should be commitInfo"
    );
