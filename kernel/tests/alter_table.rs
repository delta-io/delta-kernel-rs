...
async fn add_column_reload_snapshot_verify_schema() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(post_snap.schema().fields().count(), 3);
    ...
    // Verify email column has 2 NULLs (old rows) and 2 values (new rows)
    let email_null_count: usize = batches
        .iter()
        .map(|b| {
            b.column_by_name("email")
                .expect("email column")
                .null_count()
        })
        .sum();
    assert_eq!(email_null_count, 2);
