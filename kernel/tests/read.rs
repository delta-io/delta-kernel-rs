...
/// - Predicate only → NO stats_parsed in output (data skipping uses JSON parsing internally)
/// - stats_columns only → stats_parsed with user columns (user does their own data skipping)
/// - Both predicate AND stats_columns → stats_parsed with user columns only (data skipping works internally)
#[test]
fn scan_metadata_parsed_stats() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test 1: Without predicate or stats_columns, stats_parsed should NOT be present
    // (user must explicitly request stats via with_stats_columns)
    ...
    assert!(
        ...
        "stats_parsed should NOT be present by default (must use with_stats_columns)"
    );

    // Test 2: With predicate only, stats_parsed should NOT be present
    // (data skipping uses JSON parsing internally, but stats are not exposed to user)
    let predicate =
    ...
    assert!(
        ...
        "stats_parsed should NOT be present when only predicate is specified (data skipping is internal)"
    );
    ...
    // Test 4: Both predicate AND stats_columns should work - stats_parsed contains user columns only
    let predicate =
        delta_kernel::expressions::Predicate::gt(column_expr!("id"), Expr::literal(100i64));
    let scan = snapshot
        .scan_builder()
    ...
    let mut has_parsed_stats_combined = false;
    for scan_metadata in scan_metadata_iter {
        let scan_metadata = scan_metadata?;
        let data = scan_metadata.scan_files.data();
        let arrow_data = data
        ...
        if schema.column_with_name("stats_parsed").is_some() {
            has_parsed_stats_combined = true;
        }
    }
    assert!(
        has_parsed_stats_combined,
        "Expected stats_parsed column to be present when both predicate and stats_columns are specified"
    );
...
/// Test stats_columns with empty vector - should NOT have stats_parsed
/// (empty vector means user explicitly requested no stats)
#[test]
fn stats_columns_empty_vector() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Empty stats_columns vector means no stats requested
    let scan = snapshot
        .scan_builder()
    ...
    // Should NOT have stats_parsed (empty vector = no stats requested)
    ...
    assert!(
        !has_stats,
        "Empty stats_columns should NOT produce stats_parsed"
    );
...
/// Test with_stats_columns using None (should NOT have stats_parsed)
/// Passing None is same as not calling with_stats_columns - no stats output
#[test]
fn stats_columns_none_uses_default() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Passing None should be same as not calling with_stats_columns - no stats output
    let scan = snapshot
        .scan_builder()
    ...
    assert!(
        !has_stats,
        "None should NOT produce stats_parsed (no stats requested)"
    );
    Ok(())
}
...
/// Test predicate and stats_columns with disjoint columns.
/// Predicate is on "id", stats_columns is on "name".
/// - stats_parsed should contain ONLY "name" (not "id")
/// - Data skipping should still work on "id" predicate
#[test]
fn stats_columns_and_predicate_disjoint() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Predicate on "id", but request stats for "name" only
    let predicate =
        delta_kernel::expressions::Predicate::gt(column_expr!("id"), Expr::literal(0i64));
    let scan = snapshot
        .scan_builder()
        .with_predicate(Arc::new(predicate))
        .with_stats_columns(vec![ColumnName::new(["name"])])
        .build()?;
    ...
    for scan_metadata in scan_metadata_iter {
        ...
        // stats_parsed should be present
        let stats_parsed_idx = schema.column_with_name("stats_parsed");
        assert!(
            stats_parsed_idx.is_some(),
            "stats_parsed should be present when stats_columns is specified"
        );

        let (idx, _field) = stats_parsed_idx.unwrap();
        let stats_col = batch.column(idx);
        let stats_struct = stats_col.as_any().downcast_ref::<StructArray>().unwrap();

        // Check that minValues contains "name" but NOT "id"
        let min_values_idx = stats_struct
            .column_by_name("minValues")
            .expect("minValues should exist");
        let min_values_struct = min_values_idx.as_any().downcast_ref::<StructArray>();
        if let Some(min_struct) = min_values_struct {
            // name should be in minValues
            assert!(
                min_struct.column_by_name("name").is_some(),
                "minValues should contain 'name'"
            );
            // id should NOT be in minValues (we only requested 'name' in stats_columns)
            assert!(
                min_struct.column_by_name("id").is_none(),
                "minValues should NOT contain 'id' (only requested 'name' in stats_columns)"
            );
        }
    ...
    assert!(checked_schema, "Should have checked at least one batch");
    Ok(())
}

/// Test predicate and stats_columns with overlapping columns.
/// Both predicate and stats_columns reference "id".
/// - stats_parsed should contain "id" (not duplicated)
/// - Data skipping should work on "id"
#[test]
fn stats_columns_and_predicate_overlapping() -> Result<(), Box<dyn std::error::Error>> {
    use delta_kernel::arrow::array::StructArray;
    use delta_kernel::engine::arrow_data::ArrowEngineData;
    use delta_kernel::expressions::ColumnName;
    ...
    // Both predicate and stats_columns on "id"
    let predicate =
        delta_kernel::expressions::Predicate::gt(column_expr!("id"), Expr::literal(0i64));
    let scan = snapshot
        .scan_builder()
    ...
    for scan_metadata in scan_metadata_iter {
        ...
        // stats_parsed should be present
        let stats_parsed_idx = schema.column_with_name("stats_parsed");
        assert!(stats_parsed_idx.is_some(), "stats_parsed should be present");

        let (idx, _) = stats_parsed_idx.unwrap();
        let stats_col = batch.column(idx);
        let stats_struct = stats_col.as_any().downcast_ref::<StructArray>().unwrap();

        // Check minValues contains "id"
        if let Some(min_values) = stats_struct.column_by_name("minValues") {
            let min_struct = min_values.as_any().downcast_ref::<StructArray>();
            if let Some(min_struct) = min_struct {
                assert!(
                    min_struct.column_by_name("id").is_some(),
                    "minValues should contain 'id'"
                );
            }
        }
    ...
    assert!(checked, "Should have checked at least one batch");
    Ok(())
}
