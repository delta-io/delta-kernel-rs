...
use test_utils::{
    actions_to_string, actions_to_string_with_metadata, add_commit, add_staged_commit,
    delta_path_for_version, TestAction, CATALOG_MANAGED_METADATA,
};
...
async fn incremental_snapshot_with_log_tail() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // commits 0, 1, 2 in storage (catalog-managed table)
    let actions = vec![TestAction::Metadata];
    add_commit(
        storage.as_ref(),
        0,
