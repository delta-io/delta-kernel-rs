...
async fn test_empty_commit() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (table, engine, _store, _table_name) in
        setup_tables(schema, &[], None, "test_table").await?
    {
        assert!(matches!(
...
mod tests {
    ...
    async fn test_commit_info() -> Result<(), Box<dyn std::error::Error>> {
            ...
            setup_tables(schema, &[], None, "test_table").await?
        {
            ...
            let mut parsed_commit: serde_json::Value =
                serde_json::from_slice(&commit1.bytes().await?)?;
            *parsed_commit
                .get_mut("commitInfo")
                .unwrap()
                .get_mut("timestamp")
    ...
    async fn test_append() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (table, engine, store, table_name) in
            setup_tables(schema.clone(), &[], None, "test_table").await?
        {
            let commit_info = new_commit_info()?;
    ...
    async fn test_append_partitioned() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (table, engine, store, table_name) in
            setup_tables(table_schema.clone(), &[partition_col], None, "test_table").await?
        {
            let commit_info = new_commit_info()?;
    ...
    async fn test_append_invalid_schema() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (table, engine, _store, _table_name) in
            setup_tables(table_schema, &[], None, "test_table").await?
        {
            let commit_info = new_commit_info()?;
