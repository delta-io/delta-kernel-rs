...
pub fn get_engine(
    ...
    if args.env_creds {
        ...
        Ok(DefaultEngineBuilder::new(PrefixedStore::new(Arc::new(store), Path::from(""))).build())
    } else if !args.option.is_empty() {
        let opts = args.option.iter().map(|option| {
            let parts: Vec<&str> = option.split("=").collect();
            (parts[0].to_ascii_lowercase(), parts[1])
        });
        Ok(DefaultEngineBuilder::new(storage::store_from_url_opts(url, opts)?).build())
    } else {
        ...
        Ok(DefaultEngineBuilder::new(storage::store_from_url_opts(url, options)?).build())
    }
}
